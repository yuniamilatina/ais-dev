...
class ines_spps_trial_c extends CI_Controller {
    ...
    public function line($work_center = null) {
        ...
        $this->load->view('pes/ines_spps_trial_v', $data);
    }
    ...
    public function checkExistingWI(){
        ...
        if($data_customer->num_rows() > 0){
            $cust_part_no = $data_customer->row()->CHR_CUS_PART_NO;
            $cust_no = $data_customer->row()->CHR_CUS_NO;

            $data['flg_scan_label'] = $this->part_customer_m->get_existing_master_label($cust_part_no, $cust_no, $part_no);

            if($data['flg_scan_label'] == false){
                $master_wi = $this->part_customer_m->get_master_wi($cust_part_no, $part_no);
    ...
    public function getDetailLabel() {
        ...
        $master_wi = $this->part_customer_m->get_detail_wi_by_label($part_no_cust, $id_kanban);
