...
class Return_material_c extends CI_Controller {
    /* -- define constructor -- */
    ...
    public function index() {
        ...
        $data['content'] = 'pes/menu_location';
        $data['title'] = 'Return Material';
    ...
    public function pilihLoc(){
        ...
        redirect(base_url() . "index.php/pes/return_material_c/return_material/$pilih", "refresh");
    }
    public function return_material($pilih = null) {
        $this->role_module_m->authorization('16');
        $this->session->userdata('user_id');
        $npk=$this->session->userdata('NPK');
        $data['content'] = 'pes/return_material_v';
        $data['title'] = 'Return Material';
        $data['news'] = $this->news_m->get_news();
        $data['app'] = $this->role_module_m->get_app();
        $data['module'] = $this->role_module_m->get_module();
    ...
    function addData() {
        ...
        $jml=count($q);
        if ($jml>1) {
            $q = $this->return_material_m->getData("SELECT * FROM TM_PARTS INNER JOIN TM_KANBAN ON TM_PARTS.CHR_PART_NO = TM_KANBAN.CHR_PART_NO WHERE TM_KANBAN.CHR_KANBAN_TYPE = '0' AND TM_PARTS.CHR_BACK_NO = '$backno'"); //request pak wildan 25/05/2016
        }
    ...
    public function saveData() {
        ...
        for ($i = 0; $i < $allfix; $i++) {
            ...
            if ($intVal1=='') {
                ...
                redirect(base_url() . "index.php/pes/return_material_c/return_material/", "refresh");
            }
    ...
    public function deleteData(){
        ...
        redirect(base_url() . 'index.php/pes/return_material_c/menurm');
