...
class ng_no_locking extends CI_Controller
{
	...
	public function area($to="") {
		...
		$this->session->unset_userdata('IN_NG'); 
		
		if (!empty($to)) $this->session->set_userdata('IN_NG',$to);
		


		//var_dump($user['DEPT']);die(); 
		if ( in_array($user['DEPT'], array(21,22,23,24) ) || $to == "production" )
		{
			$data['location'] = array('WP01','WP02');
			//add by reza req by vivin
			$ltWc = $this->nolocking->execute(array('select'=>'CHR_WORK_CENTER','where'=>'CHR_PERSON_RESPONSIBLE = 01'.substr($user['DEPT'], 1, 1), 'limit'=> 1000),'tm_process'); 
			$data['wc'] = $ltWc;
			//end
		} elseif ( $user ['DEPT'] == 20 || $to == "ppic" ) {
			$data['location'] = array('WH00','PP01','PP02','PP03');
			$this->session->set_userdata('IN_NG',"ppic");// add 22/04/2016 by reza req albert
		} elseif ( $user ['DEPT'] == 6 || $to == "engineering") {//
			$data['location'] = 'EN01';
			$this->session->set_userdata('IN_NG',"engineering");// add 22/04/2016 by reza req albert
		} else {
			show_404();
		}

		$this->session->flashdata('area');
		//edit by reza req by vivin
		if(in_array($user['DEPT'], array(21,22,23,24) ) || $to == "production")
		{
			// $in = join(',', array_fill(0, count($ltWc), 'CHR_WORK_CENTER'));
			// $data['area'] = $this->nolocking->execute(array('select'=>'distinct TM_PROCESS_PARTS.CHR_AREA, TM_AREA.CHR_DESC_AREA','where'=>'CHR_WORK_CENTER IN ('.$in.')', 'join' => array('TM_AREA','TM_AREA.CHR_AREA=TM_PROCESS_PARTS.CHR_AREA')),'tm_process_parts');
			$data['area'] = $this->nolocking->execute(array('select'=>' TM_PROCESS.CHR_AREA, TM_AREA.CHR_DESC_AREA','where'=>'CHR_PERSON_RESPONSIBLE = 01'.substr($user['DEPT'], 1, 1), 'join' => array('TM_AREA','TM_AREA.CHR_AREA=TM_PROCESS.CHR_AREA')),'tm_process');			
		} elseif ($user ['DEPT'] == 6 || $to == "engineering"){
			//$data['area'] = $this->nolocking->execute(array('where'=>'CHR_FLAG_DELETE IS NULL'),'tm_area');			
			$data['area'] = $this->nolocking->execute(array('WHERE' => 'CHR_AREA = "EN-001"'), 'tm_area');
			//add 25.04.2016 by Om ezza
		}
		  elseif ($user ['DEPT'] == 20 || $to == "ppic"){
			$data['area'] = $this->nolocking->execute(array('WHERE' => 'CHR_AREA IN ("LOG-001","LOG-002","FG-001","FG-002","DEL-001","DEL-002")'), 'tm_area');
			//add 25.04.2016 by Om ezza
		}
		// end
	...
	{	
		...
		$user = $this->session->all_userdata();
		if ( isset($user['DEPT']) && !in_array($user['DEPT'], array(1,6,20)) ) show_404();

		$data['ppic'] = ($user['DEPT'] == 20 || $user['IN_NG'] == "ppic")? 1 : 0 ;
	...
	public function production()
	{
		...
		$user = $this->session->all_userdata();
		if ( !in_array($user['DEPT'], array(1, 20,21,22,23,24)) ) show_404();
		//get data session and keep session
	    $data['location'] = $this->session->flashdata('location'); 
	    $data['wc'] = $this->session->flashdata('wc');
	    // if(empty($data['location'])) redirect(site_url('pes/ng_no_locking'));
		$lvS = $this->session->flashdata('area');	    
...
redirect("fail_c");
		...
		if ($post && isset($post['load'])) {
			$data = $this->nolocking->validation();

		} elseif ( $post && isset($post['save'])) {
			if (isset($post['INT_NUMBER'])){
				$save = $this->nolocking->validation($post);
				if ($save)
				{
					$setPrint['CHR_AREA'] = $post['CHR_AREA'];	
					$setPrint['CHR_LKB_NO'] = $save['CHR_LKB_NO'];
					return $this->print_lkb($setPrint);
				}
			} else {
				if (isset($post['CHR_AREA']) && empty($post['CHR_AREA'])) $data['message'] = "Harap Memasukkan Area yang akan Divalidasi";
		...
		if ($user ['DEPT'] == 6 ){
		  ...
		  elseif ($user ['DEPT'] == 20){
			...
			// $data['area'] = $this->nolocking->execute(array('where'=>'CHR_FLAG_DELETE IS NULL'),'tm_area');
			$data['area'] = $this->nolocking->findBySql("select distinct TM_PROCESS.CHR_AREA, TM_AREA.CHR_DESC_AREA from TM_PROCESS 
											join TM_AREA
											on TM_AREA.CHR_AREA = TM_PROCESS.CHR_AREA
											where TM_PROCESS.CHR_PERSON_RESPONSIBLE =  01".substr($user['DEPT'], 1, 1)."
											and TM_AREA.CHR_FLAG_DELETE IS NULL");
		}
...
redirect("fail_c");
		...
		$data['sidebar'] = $this->role_module_m->side_bar(88);
		$this->setVars(@$data);
	}
...
redirect("fail_c");
		...
		$data['sidebar'] = $this->role_module_m->side_bar(87);
		$this->setVars(@$data);
...
redirect("fail_c");
		...
		$data['sidebar'] = $this->role_module_m->side_bar(106);
	...
	public function data($url_query='',$detail="")
	{
		...
		switch ($url_query) {
			case 'area':
				$query = array('select' => "(CHR_AREA+', '+ CHR_DESC_AREA ) as name",'LIMIT'=> 10,'or_like'=>array('CHR_AREA'=>$get,'CHR_DESC_AREA'=>$get),'where'=>'CHR_FLAG_DELETE IS NULL');
				$table = 'tm_area';
				break;
			case 'back_no':
				$query = array('select' => 'CHR_BACK_NO as name, *','distinct' => '','LIMIT'=> 10,'like' => array('CHR_BACK_NO' => $get),'where'=>'CHR_FLAG_DELETE IS NULL');
				$table = 'tm_parts';				
				break;
			case 'part_no':
				$query = array('select' => 'CHR_PART_NO as name, *','distinct' => '','LIMIT'=> 10,'like' => array('CHR_PART_NO' => $get),'where'=>'CHR_FLAG_DELETE IS NULL');
				$table = 'tm_parts';
				break;
			case 'reject_code':
				$query = array('select' => "(CHR_REJECT_CODE+', '+ CHR_REJECT_TYPE ) as name",'LIMIT'=> 10,'or_like'=>array('CHR_REJECT_CODE'=>$get,'CHR_REJECT_TYPE'=>$get),'where'=>'CHR_FLAG_DELETE IS NULL');
				$table = 'tm_reject';
				break;
			case 'ng_category':
				$query = array('select' => "(CHR_NG_CATEGORY_CODE+', '+ CHR_NG_CATEGORY ) as name",'LIMIT'=> 10,'or_like'=>array('CHR_NG_CATEGORY_CODE'=>$get,'CHR_NG_CATEGORY'=>$get),'where'=>'CHR_FLAG_DELETE IS NULL');
				$table = 'tm_ng';
				break;
			case 'damage_code':
				$query = array('select' => "(CHR_DAMAGE_CODE+', '+ CHR_DAMAGE_DESC ) as name",'LIMIT'=> 10,'or_like'=>array('CHR_DAMAGE_CODE'=>$get,'CHR_DAMAGE_DESC'=>$get),'where'=>'CHR_FLAG_DELETE IS NULL');
				$table = 'tm_damage';
				break;
			case 'chr_lkb_no':
		...
		switch ($type) {
			case 'user':
					if (isset($_POST['ng_area'])) {
						...
						if ( in_array($user['DEPT'], array(21,22,23,24) ) || isset($user['IN_NG']) && $user['IN_NG'] == "production" )
						{	
							if($this->input->post('CHR_AREA') == ""){ 
								$this->session->set_flashdata('error', 'Anda belum memasukan Area.');
								redirect( $this->agent->referrer() );
							}
							$this->session->set_flashdata('area', (object)array('CHR_AREA' => $this->input->post('CHR_AREA'), 'CHR_DESC_AREA' => $this->input->post('CHR_DESC_AREA')) );
							// $this->session->set_flashdata('area', $this->input->post('CHR_AREA'));
							$this->session->set_flashdata('wc',$this->input->post('CHR_WORK_CENTER'));
							$this->session->unset_userdata('production');
					  		redirect('pes/ng_no_locking/production');
						} elseif (in_array($user['DEPT'], array(6,20)) || isset($user['IN_NG']) && in_array($user['IN_NG'], array('engineering','ppic'))) { 
	...
	private function default_vars()
	{
	    ...
	    $this->session->keep_flashdata('wc');
	    $data['area'] = $this->session->flashdata('area');
        // load vars to view
        $this->load->vars($data);
	}
	...
	public function get_area(){
		$wc         = trim($this->input->post('wc'));
		// $val = $this->nolocking->execute(array("'where'=>'CHR_WORK_CENTER='.$wc.''"),'tm_process');
		// $val = $this->nolocking->execute(array("select'=>' TM_PROCESS.CHR_AREA, TM_AREA.CHR_DESC_AREA','where'=>'TM_PROCESS.CHR_WORK_CENTER = '".$wc."'','limit'=>'1', 'left join' => array('TM_AREA','TM_AREA.CHR_AREA=TM_PROCESS.CHR_AREA')"),'tm_process');					
		$val = $this->nolocking->findBySql("select TM_PROCESS.CHR_AREA, TM_AREA.CHR_DESC_AREA from TM_PROCESS 
								left join TM_AREA on TM_AREA.CHR_AREA=TM_PROCESS.CHR_AREA
								where TM_PROCESS.CHR_WORK_CENTER = '".$wc."'");
		$this->session->unset_userdata('area');
		$this->session->set_flashdata('area', (object)$val); 
		$data['area'] = (object)$val;
