...
class henkaten_man_c extends CI_Controller {

    private $layout = '/template/head';
    private $back_to_manage = 'henkaten/henkaten_man_c/index/';
    ...
    function index($msg = NULL) {
        $this->role_module_m->authorization('175');
        if ($msg == 1) {
        ...
        $data['data'] = $this->henkaten_man_m->get_henkaten_man();
        $data['content'] = 'henkaten/henkaten_man/manage_henkaten_man_v';
    ...
    function create() {
        ...
        $data['content'] = 'henkaten/henkaten_man/create_henkaten_man_v';
        $this->load->view($this->layout, $data);
    }

    //saving data
    function save_henkaten_man() {
        $session = $this->session->all_userdata();
        $schedule = date("Ymd", strtotime($this->input->post('CHR_SCHEDULE_DATE')));
        $actual = date("Ymd", strtotime($this->input->post('CHR_ACT_DATE')));
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
                ...
                'CHR_SCHEDULE_DATE' => $schedule,
                'CHR_ACT_DATE' => $actual,
                'CHR_CREATED_BY' => $session['USERNAME']
            );
            $this->henkaten_man_m->save($data);
    ...
    //prepare to editing
    function edit_henkaten_man($id) {
        $this->role_module_m->authorization('175');
        $data['data'] = $this->henkaten_man_m->get_henkaten_man_by_id($id);

        $data['content'] = 'henkaten/henkaten_man/edit_henkaten_man_v';
    ...
    function update_henkaten_man() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->henkaten_man_m->update($data);
    ...
    //deleting data
    function delete_henkaten_man($id) {
        $this->role_module_m->authorization('175');
        $this->henkaten_man_m->delete($id);
