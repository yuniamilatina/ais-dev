...
class henkaten_machine_c extends CI_Controller {

    private $layout = '/template/head';
    private $back_to_manage = 'henkaten/henkaten_machine_c/index/';
    ...
    function index($msg = NULL) {
        $this->role_module_m->authorization('174');
        if ($msg == 1) {
        ...
        $data['title'] = 'Manage Henkaten Machine';
        $data['msg'] = $msg;

        $data['data'] = $this->henkaten_machine_m->get_henkaten_machine();
        $data['content'] = 'henkaten/henkaten_machine/manage_henkaten_machine_v';
    ...
    function create() {
        ...
        $data['title'] = 'Create Henkaten Machine';
        $data['content'] = 'henkaten/henkaten_machine/create_henkaten_machine_v';
        $this->load->view($this->layout, $data);
    }

    //saving data
    function save_henkaten_machine() {
        ...
        $schedule_date = date("Ymd", strtotime($this->input->post('CHR_SCHEDULE_DATE')));
        $actual_date = date("Ymd", strtotime($this->input->post('CHR_ACT_DATE')));
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
                ...
                'INT_FLG_NG' => $this->input->post('INT_FLG_NG'),
                'CHR_PIC' => $this->input->post('CHR_PIC'),
            ...
            $this->henkaten_machine_m->save($data);
    ...
    //prepare to editing
    function edit_henkaten_machine($id) {
        $this->role_module_m->authorization('174');
        $data['data'] = $this->henkaten_machine_m->get_henkaten_machine_by_id($id);

        $data['content'] = 'henkaten/henkaten_machine/edit_henkaten_machine_v';
    ...
    //updating data
    function update_henkaten_machine() {
        $session = $this->session->all_userdata();
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->henkaten_machine_m->update($data);
    ...
    //deleting data
    function delete_henkaten_machine($id) {
        $this->role_module_m->authorization('174');
        $this->henkaten_machine_m->delete($id);
