<?php
	...
	* 
	*/
	class product_category_c extends CI_Controller
	{
		
		private $layout = '/template/head';
	    private $back_to_manage = 'company_profile/product_category_c/index/';

	    public function __construct() {
	        parent::__construct();
	        $this->load->model('company_profile/product_category_m');
	        // $this->load->model('asset/product_category_m');
	    }
	    ...
	    function index($msg = NULL) {
	        ...
	        $data['msg'] = $msg;
	        $data['data'] = $this->product_category_m->get_product_category();
	        $data['content'] = 'company_profile/product_category/manage_product_category_v';
	    ...
	    function create_product_category() {
	        $this->role_module_m->authorization('3');
	        $data['content'] = 'company_profile/product_category/create_product_category_v';
	    ...
	    function save_product_category() {
			...
			{
				if($size <= $maxsize){
					//move_uploaded_file($file_tmp,DOCROOT.'/assets/img/category/'.$namaF);
					move_uploaded_file($file_tmp,DOCUP.'/category/'.$namaF);
					$data = array(
		            	'CHR_CATEGORY_NAME' => $this->input->post('CHR_CATEGORY_NAME'),
		            	'CHR_CATEGORY_PHOTO' => $namaF,
		            	'INT_STATUS' => 1
		            );
					$this->product_category_m->save_product_category($data);
	    ...
	    function edit_product_category($id) {
	        $this->role_module_m->authorization('3');
	        $data['data'] = $this->product_category_m->get_data_product_category($id)->row();
	        $data['content'] = 'company_profile/product_category/edit_product_category_v';
	    ...
	    function update_product_category() {
			...
			if($namaF == ""){
				$namaF = $this->input->post('CHR_CATEGORY_PHOTO_OLD');
				$data = array(
	            	'CHR_CATEGORY_NAME' => $this->input->post('CHR_CATEGORY_NAME'),
	            	'CHR_CATEGORY_PHOTO' => $namaF,
	            	'INT_STATUS' => 1
	            );
				$this->product_category_m->update($data, $id);
			...
			{
				if($size <= $maxsize){
					//move_uploaded_file($file_tmp,DOCIMG.'/image/category/'.$namaF);
					move_uploaded_file($file_tmp,DOCUP.'/category/'.$namaF);
					$data = array(
		            	'CHR_CATEGORY_NAME' => $this->input->post('CHR_CATEGORY_NAME'),
		            	'CHR_CATEGORY_PHOTO' => $namaF,
	    ...
	    function delete_product_category($id) {
	        $this->role_module_m->authorization('3');
	        $this->product_category_m->delete($id);
