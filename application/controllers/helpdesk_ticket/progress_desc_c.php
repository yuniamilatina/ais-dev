...
class progress_desc_c extends CI_Controller {

    private $layout = '/template/head';
    private $back_to_manage = 'helpdesk_ticket/progress_desc_c/index/';
    ...
    function save_prover() {
        $this->form_validation->set_rules('CHR_PROVER', 'Problem Solver Initial', 'required|min_length[4]|max_length[4]|callback_check_id_prover|trim');
    ...
    function add_progress() {
        ...
        $id_ticket = $this->input->post('INT_ID_TICKET');
        $id_prover = $this->prover_m->get_id_prover_by_npk($session['NPK']);

        $data = array(
            'INT_ID_TICKET' => $id_ticket,
            'INT_ID_PROVER' => $id_prover,
            'CHR_PROGRESS_DESC' => trim($this->input->post('CHR_PROGRESS_DESC')),
        ...
        $this->progress_desc_m->save_progress_desc($data);
        $this->helpdesk_ticket_m->feedback_ticket($id_ticket);
        redirect('helpdesk_ticket/helpdesk_ticket_c/prepare_to_feedback_helpdesk_ticket/' . $id_ticket . '/' . $msg = 1);
    }
     ...
     function edit_progress() {
        ...
        $id_ticket = $this->input->post('INT_ID_TICKET');
        $id_progress = $this->input->post('INT_ID_PROGRESS');
        $id_prover = $this->prover_m->get_id_prover_by_npk($session['NPK']);
        ...
        $this->progress_desc_m->update_progress_desc($data, $id_progress);
        redirect('helpdesk_ticket/helpdesk_ticket_c/prepare_to_feedback_helpdesk_ticket/' . $id_ticket . '/' . $msg = 2);
    }

    function not_close_ticket($id_progress, $id_ticket) {
        $this->helpdesk_ticket_m->not_close_ticket($id_ticket);
        $this->progress_desc_m->not_close_ticket($id_progress);
        redirect('helpdesk_ticket/helpdesk_ticket_c/prepare_to_feedback_helpdesk_ticket/' . $id_ticket . '/' . $msg = 3);
    }

    function close_ticket($id_progress, $id_ticket) {
        $this->helpdesk_ticket_m->close_ticket($id_ticket);
        $this->progress_desc_m->close_ticket($id_progress);
        redirect('helpdesk_ticket/helpdesk_ticket_c/prepare_to_feedback_helpdesk_ticket/' . $id_ticket . '/' . $msg = 3);
    }
    ...
    function update_prover() {
        $this->form_validation->set_rules('CHR_PROVER', 'Problem Solver Initial', 'required|min_length[4]|max_length[4]|trim');
        $this->form_validation->set_rules('CHR_PROVER_DESC', 'Prover Description', 'required|max_length[20]|trim');
        $session = $this->session->all_userdata();
        if ($this->form_validation->run() == FALSE) {
    ...
    function moving_ticket() {
        ...
        if ($checked == null) {
            redirect('helpdesk_ticket/progress_desc_c/view_detail_prover/' . $id_prover . '/' . 4);
        }

        for ($i = 0; $i < count($checked); $i++) {
            $this->helpdesk_ticket_m->update_helpdesk_ticket($data, $checked[$i]);
        }
        redirect('helpdesk_ticket/progress_desc_c/view_detail_prover/' . $id_prover . '/' . 1);
    }
