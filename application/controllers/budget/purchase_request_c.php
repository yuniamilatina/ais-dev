...
class purchase_request_c extends CI_Controller {
    ...
    function approval_purchase_request($msg = NULL, $tahun = NULL, $budget_type = NULL, $status_bgt = NULL, $kode_transaksi = NULL) {
        ...
        if($tahun == NULL){
            // $tahun = $this->db->query("SELECT TOP 1 INT_ID_FISCAL_YEAR FROM CPL.TM_FISCAL WHERE BIT_FLG_DEL = 0 ORDER BY INT_ID_FISCAL_YEAR ASC")->row()->INT_ID_FISCAL_YEAR;
            $tahun = $this->db->query("SELECT TOP 1 INT_ID_FISCAL_YEAR FROM CPL.TM_FISCAL WHERE BIT_FLG_ACTIVE = 1 ORDER BY INT_ID_FISCAL_YEAR ASC")->row()->INT_ID_FISCAL_YEAR;
            // $tahun = date("Y");
        }
        ...
        //FOR --> BOD
            ...
            $data['total_all_budget_plan_dept'] = $this->purchase_request_m->get_total_budget_plan($tahun, $budget_type, $kode_dept);
            //BY RATIO AFTER CR
            $data['revisi_budget_by_sales_dept'] = $this->purchase_request_m->get_new_budget_detail_rev_by_sales($tahun, $budget_type, $kode_dept);
            
            //TOTAL BUDGET REVISI PER DEPT
            $data['total_all_budget_revisi_dept'] = $this->purchase_request_m->get_total_all_budget_revisi($tahun, $budget_type, $kode_dept);
        ...
        //FOR --> GM    
            ...
            //TOTAL BUDGET PLANT
            $data['total_budget_plant'] = $this->purchase_request_m->get_total_budget_plant($tahun, $budget_type)->TOT_BUDGET_PLANT;            
            
            //TOTAL BUDGET REVISI PLANT
            $data['total_all_budget_revisi_plant'] = $this->purchase_request_m->get_total_budget_revisi_plant($tahun, $budget_type)->TOT_BGTREV;
        ...
        //FOR --> MANAGER
        } else if ($session['ROLE'] === 5 || $session['ROLE'] === 39 || $session['ROLE'] === 45 || $session['ROLE'] === 10) {
