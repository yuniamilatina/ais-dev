...
class budget_nopin_c extends CI_Controller {

    private $layout = '/template/head';
    private $layout_blank = '/template/head_blank';
    private $back_to_upload = 'budget/budget_nopin_c/create_nopin/';
    private $back_to_manage = 'budget/budget_nopin_c/index/';

    public function __construct() {
        parent::__construct();
        $this->load->model('budget/budget_nopin_m');
    ...
    function create_nopin($msg = null, $INT_ID_FISCAL_YEAR = null, $INT_ID_BUDGET_TYPE = null, $INT_DIV = null, $INT_DEPT = null, $INT_SECT = null, $CHR_BUDGET_TYPE = null) {
        ...
        $data['INT_ID_BUDGET_TYPE'] = 52;
        $data['INT_DIV'] = $INT_DIV = 1;
        $data['INT_DEPT'] = $INT_DEPT = 8;
        $data['INT_SECT'] = $INT_SECT = 20;
        $data['CHR_BUDGET_TYPE'] = 'NOPIN';                 
...
//GET DETAIL BUDGET
        if ($CHR_BUDGET_TYPE <> null) {
            $data['url_page'] = site_url("budget/budget_nopin_c/refresh_table_page/1/$INT_ID_FISCAL_YEAR/$INT_ID_BUDGET_TYPE/$INT_DIV/$INT_DEPT/$INT_SECT/$CHR_BUDGET_TYPE");
        } else {
            $data['url_page'] = "";
        }
        ...
        $data['sidebar'] = $this->role_module_m->side_bar(213);
        $data['news'] = $this->news_m->get_news();
        $data['msg'] = $msg;
        $data['data_fiscal'] = $this->fiscal_m->get_all_fiscal();
        
        $data['title'] = 'Create Budget Non Operating Income';
        $data['content'] = 'budget/budget_nopin/create_budget_nopin_v';
    ...
    function prepare_approve_nopin($msg = null, $INT_ID_FISCAL_YEAR = null, $INT_DIV = null, $INT_GROUP = null, $INT_DEPT = null) {
        $user_session = $this->session->all_userdata();
        if ($msg == 5) {
            $msg = "<div class = 'alert alert-success'><button type = 'button' class = 'close' data-dismiss = 'alert'>×</button><strong>Commiting success. </strong> The data is successfully updated. </div >";
        } elseif ($msg == 6) {
        ...
        if($INT_ID_FISCAL_YEAR <> null){
            $data['url_page'] = site_url("budget/budget_nopin_c/refresh_detail_table_page/1/$INT_ID_FISCAL_YEAR/$INT_DIV/$INT_GROUP/$INT_DEPT");
        } else {
            $data['url_page'] = "";
        }
        ...
        $data['module'] = $this->role_module_m->get_module();
        $data['function'] = $this->role_module_m->get_function();
        $data['sidebar'] = $this->role_module_m->side_bar(216);
        $data['news'] = $this->news_m->get_news();
        $data['data_fiscal'] = $this->fiscal_m->get_all_fiscal();         

        $data['title'] = 'Approval for Planning Non Operating Income';
        $data['msg'] = $msg;        
        ...
        $data['content'] = 'budget/budget_nopin/approval_budget_nopin_v';
    ...
    function refresh_detail_table() {
        ...
        $url_iframe = site_url("budget/budget_nopin_c/refresh_detail_table_page/1/$INT_ID_FISCAL_YEAR/$INT_DIV/$INT_GROUP/$INT_DEPT");
        $url_export_excel = site_url("budget/budget_nopin_c/download_excel_for_approve/$INT_ID_FISCAL_YEAR/$INT_DIV/$INT_GROUP/$INT_DEPT");
...
//Either you can print value or you can send value to database
        ...
        //echo site_url("budget/budget_nopin_c/refresh_detail_table_page/1/$INT_ID_FISCAL_YEAR/$INT_DIV/$INT_GROUP/$INT_DEPT");
    }
    ...
    function refresh_detail_table_page($msg = null, $INT_ID_FISCAL_YEAR = null, $INT_DIV = null, $INT_GROUP = null, $INT_DEPT = null) {
        ...
        $data['content'] = 'budget/budget_nopin/refresh_detail_budget_nopin_v';
        
        $data['summary_nopin'] = $this->budget_nopin_m->get_summary_nopin($INT_ID_FISCAL_YEAR, $INT_DIV, $INT_GROUP, $INT_DEPT);
        $data['summary_nopin_total'] = $this->budget_nopin_m->get_summary_nopin_total($INT_ID_FISCAL_YEAR, $INT_DIV, $INT_GROUP, $INT_DEPT);
        ...
        $data['title'] = 'Approval Budget Non Operating Income';

        $this->load->view($this->layout_blank, $data);
    }

    function approve_budget_nopin() {
        $user_session = $this->session->all_userdata();
        ...
        if ($_POST["btn-save"] == 'man') {
            ...
            $app_man->query("UPDATE CPL.TT_BUDGET_NON_OPERATING_INCOME SET CHR_FLAG_APP_MAN = '1'
            ...
            if ($app_man->trans_status() === FALSE) {
                ...
                redirect("budget/budget_nopin_c/prepare_approve_nopin/12/$INT_ID_FISCAL_YEAR/$INT_DIV/$INT_GROUP/$INT_DEPT");
            } else {
                $app_man->trans_commit();
                redirect("budget/budget_nopin_c/prepare_approve_nopin/6/$INT_ID_FISCAL_YEAR/$INT_DIV/$INT_GROUP/$INT_DEPT");
            }
        } else if ($_POST["btn-save"] == 'gm') {
            $app_gm = $this->db;
            $app_gm->trans_start();

            $app_gm->query("UPDATE CPL.TT_BUDGET_NON_OPERATING_INCOME SET CHR_FLAG_APP_GM = '1'
                            WHERE (INT_ID_FISCAL_YEAR = '$INT_ID_FISCAL_YEAR')
                                    AND (CHR_STAT_REV = '$CHR_STAT_REV')
                                    AND (INT_DIV = '$INT_DIV')
                                    AND (INT_GROUP_DEPT = '$INT_GROUP')
            ...
            $app_dir->query("UPDATE CPL.TT_BUDGET_NON_OPERATING_INCOME SET CHR_FLAG_APP_DIR = '1'
                            WHERE (INT_ID_FISCAL_YEAR = '$INT_ID_FISCAL_YEAR')
                                    AND (CHR_STAT_REV = '$CHR_STAT_REV')
                                    AND (INT_DIV = '$INT_DIV')
                                    AND (INT_GROUP_DEPT = '$INT_GROUP')
            ...
            $app_all->query("UPDATE CPL.TT_BUDGET_NON_OPERATING_INCOME SET CHR_FLAG_APP_MAN = '1', CHR_FLAG_APP_GM = '1', CHR_FLAG_APP_DIR = '1', CHR_FLAG_APP_COMPLETE = '1'
                            WHERE (INT_ID_FISCAL_YEAR = '$INT_ID_FISCAL_YEAR')
                                    AND (CHR_STAT_REV = '$CHR_STAT_REV')
                                    AND (INT_DIV = '$INT_DIV')
                                    AND (INT_GROUP_DEPT = '$INT_GROUP')
            ...
            $app_all->query("UPDATE CPL.TT_BUDGET_NON_OPERATING_INCOME SET CHR_FLAG_APP_MAN = '0', CHR_FLAG_APP_GM = '0', CHR_FLAG_APP_DIR = '0', CHR_FLAG_APP_COMPLETE = '0'
                            WHERE (INT_ID_FISCAL_YEAR = '$INT_ID_FISCAL_YEAR')
                                    AND (CHR_STAT_REV = '$CHR_STAT_REV')
                                    AND (INT_DIV = '$INT_DIV')
                                    AND (INT_GROUP_DEPT = '$INT_GROUP')
            ...
            if ($app_all->trans_status() === FALSE) {
                ...
                $app_all->trans_commit();
                redirect("budget/budget_nopin_c/prepare_approve_nopin/7/$INT_ID_FISCAL_YEAR/$INT_DIV/$INT_GROUP/$INT_DEPT");
            }
        }
    }
    ...
    function download_template($budget_type) {
        ...
        $filename = 'Template_Non_Operating_Income';
    ...
    function upload_budget_nopin() {
        ...
        $this->budget_nopin_m->delete_existing_template($CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);

        $upload_date = date('Ymd');
        $fileName = $_FILES['upload_budget_nopin']['name'];
        if (empty($fileName)) {
            redirect($this->back_to_upload . $msg = 12);
        }

        //file untuk submit file excel
        $config['upload_path'] = './assets/file/budget_non_operating_income/';
        ...
        if ($a = $this->upload->do_upload('upload_budget_nopin'))
            $this->upload->display_errors();
        $media = $this->upload->data('upload_budget_nopin');
        ...
        $inputFileName = './assets/file/budget_non_operating_income/' . $media['file_name'];
...
//SAVE TO DATABASE
            $this->budget_nopin_m->save_temp($data);
        }
        redirect("budget/budget_nopin_c/confirmation_budget_nopin/$INT_ID_FISCAL_YEAR/$INT_ID_BUDGET_TYPE/$INT_DIV/$INT_DEPT/$INT_SECT/$CHR_BUDGET_TYPE", "REFRESH");
    }

    function confirmation_budget_nopin($INT_ID_FISCAL_YEAR, $INT_ID_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT, $CHR_BUDGET_TYPE) {
        $user_session = $this->session->all_userdata();
        ...
        $data['news'] = $this->news_m->get_news();
        $data['title'] = 'Confirmation Budget Non Operating Income';
        $data['subcontent'] = NULL;
        $data['content'] = 'budget/budget_nopin/confirmation_budget_nopin_v';
        ...
        //GET DETAIL BUDGET
        $data['detail_confirm'] = $this->budget_nopin_m->get_detail_confirm_nopin($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
        $data['detail_confirm_sum'] = $this->budget_nopin_m->get_sum_nopin($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
        //GET SUM AMOUNT
        $data['SUM_AMT'] = $this->budget_nopin_m->get_sum_amt_confirm_nopin($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
    ...
    function save_budget($INT_ID_FISCAL_YEAR, $INT_ID_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT, $CHR_BUDGET_TYPE, $CHR_STAT_REV) {
        ...
        $this->budget_nopin_m->delete_existing_budget($INT_ID_FISCAL_YEAR, $INT_ID_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT, $CHR_BUDGET_TYPE, $CHR_STAT_REV);

        //CHECK SEQUNCE BUDGET NUMBER
        //GET BUDGET NUMBER
        //SAVE DATA FROM TABLE WORK TO TABLE TRANSACTION
        $detail_budget = $this->budget_nopin_m->get_detail_confirm_nopin($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
        //ASSIGN TO ARRAY
        foreach ($detail_budget as $value) {
            $CHR_NO_BUDGET = $this->budget_nopin_m->get_no_budget($INT_ID_FISCAL_YEAR, $INT_ID_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT, $CHR_BUDGET_TYPE, $CHR_STAT_REV);
            $data = array(
        ...
        redirect("budget/budget_nopin_c/create_nopin/1/$INT_ID_FISCAL_YEAR/$INT_ID_BUDGET_TYPE/$INT_DIV/$INT_DEPT/$INT_SECT/$CHR_BUDGET_TYPE", "REFRESH");
        $this->load->view($this->layout, $data);
    }
    ...
    function refresh_table() {
        ...
        $get_gm_div = $this->dept_m->get_gm_div($INT_DEPT)->row();
        $INT_DIV = $get_gm_div->INT_ID_DIVISION;
        $url_iframe = site_url("budget/budget_nopin_c/refresh_table_page/1/$INT_ID_FISCAL_YEAR/$INT_BUDGET_TYPE/$INT_DIV/$INT_DEPT/$INT_SECT/$CHR_BUDGET_TYPE");
        $url_export_excel = site_url("budget/budget_nopin_c/download_excel/$INT_ID_FISCAL_YEAR/$INT_BUDGET_TYPE/$INT_DIV/$INT_DEPT/$INT_SECT/$CHR_BUDGET_TYPE");
        
        $status_approve_gm = $this->budget_nopin_m->get_status_approve_gm($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT);
...
//Either you can print value or you can send value to database
        ...
        //echo site_url("budget/budget_nopin_c/refresh_table_page/1/$INT_ID_FISCAL_YEAR/$INT_BUDGET_TYPE/$INT_DIV/$INT_DEPT/$INT_SECT/$CHR_BUDGET_TYPE");
    }
...
//GET DETAIL BUDGET
        if ($CHR_BUDGET_TYPE <> null) {
            $data['detail_confirm'] = $this->budget_nopin_m->get_detail_nopin($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
            $data['detail_confirm_sum'] = $this->budget_nopin_m->get_sum_nopin($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
        } else {
        ...
        $data['subcontent'] = NULL;
        $data['content'] = 'budget/budget_nopin/refresh_budget_nopin_v';
    ...
    function download_excel_for_approve($INT_ID_FISCAL_YEAR = null, $INT_DIV = null, $INT_GROUP = null, $INT_DEPT = null) {
        $row = 8;
        $INT_ID_BUDGET_TYPE = 52;
        $detail_confirm = $this->budget_nopin_m->get_summary_nopin($INT_ID_FISCAL_YEAR, $INT_DIV, $INT_GROUP, $INT_DEPT);
        ...
        $objPHPExcel = $objReader->load("assets/template/budget/report/Template_Approval_Non_Operating_Income.xls");
    ...
    function download_excel($INT_ID_FISCAL_YEAR = null, $INT_ID_BUDGET_TYPE = null, $INT_DIV = null, $INT_DEPT = null, $INT_SECT = null, $CHR_BUDGET_TYPE = null) {
        ...
        if ($CHR_BUDGET_TYPE <> null) {
            $detail_confirm = $this->budget_nopin_m->get_detail_nopin_dept($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
            $detail_confirm_sum = $this->budget_nopin_m->get_sum_nopin_dept($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
            $budget_desc = $this->budgettype_m->get_budget_type($INT_ID_BUDGET_TYPE);
            $budget_desc = strtoupper(trim($budget_desc->CHR_BUDGET_TYPE_DESC));
            ...
            $objPHPExcel = $objReader->load("assets/template/budget/report/Template_Report_Non_Operating_Income.xls");
