...
class budget_tax_c extends CI_Controller {

    private $layout = '/template/head';
    private $layout_blank = '/template/head_blank';
    private $back_to_upload = 'budget/budget_tax_c/create_tax/';
    private $back_to_manage = 'budget/budget_tax_c/index/';

    public function __construct() {
        parent::__construct();
        $this->load->model('budget/budget_tax_m');
    ...
    function create_tax($msg = null, $INT_ID_FISCAL_YEAR = null, $INT_ID_BUDGET_TYPE = null, $INT_DIV = null, $INT_DEPT = null, $INT_SECT = null, $CHR_BUDGET_TYPE = null) {
        ...
        $data['INT_ID_BUDGET_TYPE'] = 56;
        $data['INT_DIV'] = $INT_DIV = 1;
        $data['INT_DEPT'] = $INT_DEPT = 5;
        $data['INT_SECT'] = $INT_SECT = 7;
        $data['CHR_BUDGET_TYPE'] = 'Tax';
...
//GET DETAIL BUDGET
        if ($CHR_BUDGET_TYPE <> null) {
            $data['url_page'] = site_url("budget/budget_tax_c/refresh_table_page/1/$INT_ID_FISCAL_YEAR/$INT_ID_BUDGET_TYPE/$INT_DIV/$INT_DEPT/$INT_SECT/$CHR_BUDGET_TYPE");
        } else {
            $data['url_page'] = "";
        }
        ...
        $data['sidebar'] = $this->role_module_m->side_bar(214);
        $data['news'] = $this->news_m->get_news();
        $data['msg'] = $msg;
        $data['data_fiscal'] = $this->fiscal_m->get_all_fiscal();  

        $data['role'] = $user_session['ROLE'];
        $data['title'] = 'Create Budget Tax';
        $data['content'] = 'budget/budget_tax/create_budget_tax_v';
    ...
    function upload_budget_tax() {
        ...
        $this->budget_tax_m->delete_existing_template($CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);

        $upload_date = date('Ymd');
        $fileName = $_FILES['upload_budget_tax']['name'];
        if (empty($fileName)) {
            redirect($this->back_to_upload . $msg = 12);
        }

        //file untuk submit file excel
        $config['upload_path'] = './assets/file/budget_tax/';
        ...
        if ($a = $this->upload->do_upload('upload_budget_tax'))
            $this->upload->display_errors();
        $media = $this->upload->data('upload_budget_tax');
        ...
        $inputFileName = './assets/file/budget_tax/' . $media['file_name'];
...
//SAVE TO DATABASE
            $this->budget_tax_m->save_temp($data);
        }
        redirect("budget/budget_tax_c/confirmation_budget_tax/$INT_ID_FISCAL_YEAR/$INT_ID_BUDGET_TYPE/$INT_DIV/$INT_DEPT/$INT_SECT/$CHR_BUDGET_TYPE", "REFRESH");
    }

    function confirmation_budget_tax($INT_ID_FISCAL_YEAR, $INT_ID_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT, $CHR_BUDGET_TYPE) {
        $user_session = $this->session->all_userdata();
        ...
        $data['news'] = $this->news_m->get_news();
        $data['title'] = 'Confirmation Budget Tax';
        $data['subcontent'] = NULL;
        $data['content'] = 'budget/budget_tax/confirmation_budget_tax_v';
        ...
        //GET DETAIL BUDGET
        $data['detail_confirm'] = $this->budget_tax_m->get_detail_confirm_tax($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
        $data['detail_confirm_sum'] = $this->budget_tax_m->get_sum_tax($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
        //GET SUM AMOUNT
        $data['SUM_AMT'] = $this->budget_tax_m->get_sum_amt_confirm_tax($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
    ...
    function save_budget($INT_ID_FISCAL_YEAR, $INT_ID_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT, $CHR_BUDGET_TYPE, $CHR_STAT_REV) {
        ...
        $this->budget_tax_m->delete_existing_budget($INT_ID_FISCAL_YEAR, $INT_ID_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT, $CHR_BUDGET_TYPE, $CHR_STAT_REV);

        //CHECK SEQUNCE BUDGET NUMBER
        //GET BUDGET NUMBER
        //SAVE DATA FROM TABLE WORK TO TABLE TRANSACTION
        $detail_budget = $this->budget_tax_m->get_detail_confirm_tax($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
        //ASSIGN TO ARRAY
        foreach ($detail_budget as $value) {
            $CHR_NO_BUDGET = $this->budget_tax_m->get_no_budget($INT_ID_FISCAL_YEAR, $INT_ID_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT, $CHR_BUDGET_TYPE, $CHR_STAT_REV);
            $data = array(
        ...
        redirect("budget/budget_tax_c/create_tax/1/$INT_ID_FISCAL_YEAR/$INT_ID_BUDGET_TYPE/$INT_DIV/$INT_DEPT/$INT_SECT/$CHR_BUDGET_TYPE", "REFRESH");
        $this->load->view($this->layout, $data);
    }
    ...
    function refresh_table() {
        ...
        $get_gm_div = $this->dept_m->get_gm_div($INT_DEPT)->row();
        $INT_DIV = $get_gm_div->INT_ID_DIVISION;
        $url_iframe = site_url("budget/budget_tax_c/refresh_table_page/1/$INT_ID_FISCAL_YEAR/$INT_BUDGET_TYPE/$INT_DIV/$INT_DEPT/$INT_SECT/$CHR_BUDGET_TYPE");
        $url_export_excel = site_url("budget/budget_tax_c/download_excel/$INT_ID_FISCAL_YEAR/$INT_BUDGET_TYPE/$INT_DIV/$INT_DEPT/$INT_SECT/$CHR_BUDGET_TYPE");
...
//Either you can print value or you can send value to database
        ...
        //echo site_url("budget/budget_tax_c/refresh_table_page/1/$INT_ID_FISCAL_YEAR/$INT_BUDGET_TYPE/$INT_DIV/$INT_DEPT/$INT_SECT/$CHR_BUDGET_TYPE");
    }
...
//GET DETAIL BUDGET
        if ($CHR_BUDGET_TYPE <> null) {
            $data['detail_confirm'] = $this->budget_tax_m->get_detail_tax($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
            $data['detail_confirm_sum'] = $this->budget_tax_m->get_sum_tax($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
        } else {
        ...
        $data['subcontent'] = NULL;
        $data['content'] = 'budget/budget_tax/refresh_budget_tax_v';
    ...
    function download_excel($INT_ID_FISCAL_YEAR = null, $INT_ID_BUDGET_TYPE = null, $INT_DIV = null, $INT_DEPT = null, $INT_SECT = null, $CHR_BUDGET_TYPE = null) {
        ...
        if ($CHR_BUDGET_TYPE <> null) {
            $detail_confirm = $this->budget_tax_m->get_detail_tax_dept($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
            $detail_confirm_sum = $this->budget_tax_m->get_sum_tax_dept($INT_ID_FISCAL_YEAR, $CHR_BUDGET_TYPE, $INT_DIV, $INT_DEPT, $INT_SECT);
