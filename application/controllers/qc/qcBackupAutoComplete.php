    	...
    	if($jaga_query->num_rows() > 0){
	    	...
	    	if($del_query->num_rows() > 0){
	    		...
	    		foreach ($del_item as $value) {
	    			...
	    			$data.='<td class="tg-031e" style="text-align:center"><input type="text" onkeypress="return (event.charCode >= 48 && event.charCode <= 57)||event.charCode <= 8" style="text-align:center; height:35px" width="100" class="form-control" readonly="readonly" name="partno_'.$x.'" id="partno_'.$x.'" value="'.$partnobaru.'" /></td>';
	    			...
	    			$data.='<td class="tg-031e" style="text-align:center"><input type="text" onkeypress="return (event.charCode >= 48 && event.charCode <= 57)||event.charCode <= 8" style="text-align:center; height:35px" width="100" class="form-control" readonly="readonly" name="del_'.$x.'" id="del_'.$x.'" value="'.$value->INT_TOTAL_QTY.'" /></td>';
	    			...
	    			$data.='<td class="tg-031e" align="center" id="field_'.$x.'" name="field_'.$x.'"><input type="text" style="width:100px; text-align:center; background-color:red; color:white" class="form-control" name="status_'.$x.'" id="status_'.$x.'" value="NG" readonly="readonly"/></td>';
	    		...
	    		$data = '<div style="padding-left:20px; background-color:white; font-size:12px; font-style:bold"> Maaf, Data tidak ditemukan.</div>';
    ...
    public function saveData(){
        ...
        $time = date('Hms');
	        ...
	        if ($scrap_query->num_rows() >= 0){
	        	...
	        	$this->db->query("INSERT INTO TT_NG_RECORD_H(CHR_PLANT, CHR_LKB_NO, CHR_LKB_DATE, CHR_LKB_NUM,INT_NPK_MSU,CHR_IP_MSU,CHR_USER_MSU,CHR_STATUS, CHR_PRINT_STATUS, CHR_AREA) VALUES('600','".$no_lkb."','".$date_now."','".$no."','".$npk."','".$ip."','".$username."','1','1','FG-004')");
...
//        $this->dompdf->stream("welcome.pdf");
    ...
    public function getTableDOCancel(){
    	...
    	// $jaga_query = $this->db->query("SELECT CHR_DEL_NO FROM TT_DELIVERY WHERE CHR_DEL_NO = '$no_do' AND CHR_DEL_TYPE = 'ZRFM'");
    	$jaga_query = $this->db->query("SELECT INT_QCE_NO FROM TT_QCE_H WHERE CHR_DEL_NO = '$no_do' AND CHR_CANCEL_STATUS != '1'");
        ...
        $time = date('Hms');
	        ...
	        for($x = 1; $x < $no; $x++){
	            ...
	            if ($good_qty <> 0){
	                $this->db->query("INSERT INTO TT_GOODS_MOVEMENT_H(CHR_PLANT,CHR_DATE,
	                CHR_TYPE_TRANS,CHR_DOCUMENT_DATE,CHR_MVMT_TYPE, CHR_IP, CHR_USER, CHR_NPK, CHR_VALIDATE,CHR_UPLOAD,
	                CHR_STATUS,CHR_REMARKS) VALUES('600','".$date."','GDDL','".$date."','343', '".$ip."', '".$username."', '".$npk."','X','0','0','".$no_do."')");
	            ...
	            if ($repair_qty <> 0){
	            	$this->db->query("INSERT INTO TT_GOODS_MOVEMENT_H(CHR_PLANT,CHR_DATE,
	                CHR_TYPE_TRANS,CHR_DOCUMENT_DATE,CHR_MVMT_TYPE,CHR_IP,CHR_USER,CHR_NPK,CHR_VALIDATE,CHR_UPLOAD,
	                CHR_STATUS,CHR_REMARKS) VALUES('600','".$date."','RPDL','".$date."','325','".$ip."','".$username."','".$npk."','X','0','0','".$no_do."')");
	            ...
	            if ($claim_qty <> 0){
	            	$this->db->query("INSERT INTO TT_GOODS_MOVEMENT_H(CHR_PLANT,CHR_DATE,
	                CHR_TYPE_TRANS,CHR_DOCUMENT_DATE,CHR_MVMT_TYPE,CHR_IP,CHR_USER,CHR_NPK,CHR_VALIDATE,CHR_UPLOAD,
	                CHR_STATUS,CHR_REMARKS) VALUES('600','".$date."','CLDL','".$date."','325','".$ip."','".$username."','".$npk."','X','0','0','".$no_do."')");
