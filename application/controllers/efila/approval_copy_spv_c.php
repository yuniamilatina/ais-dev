<?php
	...
	* 
	*/
	class approval_copy_spv_c extends CI_Controller
	{
		
		private $layout = '/template/head';
	    private $back_to_manage = 'efila/approval_copy_spv_c/index/';
	    ...
	    function index($msg = NULL) {
	        ...
	        $data['msg'] = $msg;
	        $data['data'] = $this->approval_copy_document_m->get_copy_spv();
	        $data['content'] = 'efila/approval_spv/approval_copy_v';
	    ...
	    function detail_copy($id) {
	        ...
	        $data['content'] = 'efila/approval_spv/copy_detail_v';
	    ...
	    //updating data
	    function update_approval_spv($status, $id = NULL, $iddoc = NULL) {
	        $msg = 2;
			...
			if($this->input->post('CHR_INFO')) {
				...
				$data = array(
	            	'INT_APPROVED_SPV' => $status,
	            	'CHR_APPROVED_SPV_DATE' => $tgl,
	            	'CHR_APPROVED_SPV_TIME' => $time,
	            	'CHR_APPROVED_SPV_BY' => $this->session->userdata("USERNAME"),
	            ...
	            $data1 = array(
	        		...
	        		'CHR_NOTIF_DESC' => "Copy Document Rejected By Spv",
            ...
            redirect('efila/approval_copy_spv_c/index/'. $msg);					
	    }
