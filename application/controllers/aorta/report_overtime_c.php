...
class report_overtime_c extends CI_Controller {
    ...
    public function overtime($periode = null, $id_dept = null, $id_section = null, $id_subsect = null) {
        ...
        $data['sidebar'] = $this->role_module_m->side_bar(165);
        $data['news'] = $this->news_m->get_news();        
        $data['content'] = 'aorta/report_overtime_v';
        ...
        if ($this->session->userdata('ROLE') == 5 || $this->session->userdata('ROLE') == 6 || $this->session->userdata('ROLE') == 39 || $this->session->userdata('ROLE') == 45) { // Manager
            
            $kode_dept = $this->session->userdata('DEPT');
            
            $data['overtime_quota'] = $this->master_prod_target_m->get_overtime_quota($selected_date, $kode_dept, $id_section, $id_subsect);
            $data['overtime_summary'] = $this->master_prod_target_m->get_overtime_summary_all($selected_date, $kode_dept, $id_section, $id_subsect);

        } else if ($this->session->userdata('ROLE') == 4 ) { //GM or Director or Root or Task Force
            ...
            $data['overtime_quota'] = $this->master_prod_target_m->get_overtime_quota($selected_date, $kode_dept, $id_section, $id_subsect);
            $data['overtime_summary'] = $this->master_prod_target_m->get_overtime_summary_all($selected_date, $kode_dept, $id_section, $id_subsect);
        } else if ($this->session->userdata('ROLE') == 1 || $this->session->userdata('ROLE') == 3 || $this->session->userdata('ROLE') == 62 || $this->session->userdata('ROLE') == 14) { //GM or Director or Root or Task Force
        ...
        $data['all_work_centers'] = $this->master_prod_target_m->get_section($kode_dept);

        if ($id_section == null || $id_section == 'ALL' || $id_section == '') {
            $choose_work_centers = $data['all_work_centers'];
            $choose_line = $this->master_prod_target_m->get_overtime_summary_all($selected_date, $kode_dept, $id_section = null, $id_subsect = null);
        } else {
            if($id_subsect == null || $id_subsect == '' || $id_subsect == 'ALL'){
                $choose_work_centers = $this->master_prod_target_m->get_section_detail($kode_dept, $id_section);
                $choose_line = $this->master_prod_target_m->get_overtime_summary_all($selected_date, $kode_dept, $id_section, $id_subsect = null);
            }else{
                $choose_work_centers = $this->master_prod_target_m->get_section_detail($kode_dept, $id_section);
                $choose_line = $this->master_prod_target_m->get_overtime_summary_all($selected_date, $kode_dept, $id_section, $id_subsect);
            }
        }
        
        $data['holiday'] = $this->master_prod_target_m->get_holiday($selected_date);
        $data['id_prod'] = $kode_dept;
        
        $data['choose_work_centers'] = $choose_work_centers;
        $data['choose_line'] = $choose_line;
        // $data['all_dept_prod'] = $this->dept_m->get_all_prod_dept(); //===== Khusus prod
        $data['all_dept_prod'] = $this->dept_m->get_all_dept_plant();
        $data['all_lines'] = $this->master_prod_target_m->get_sub_section($kode_dept, $id_section);
    ...
    public function print_report_overtime() {
        ...
        if($dept_selected == '' || $dept_selected == null){
            $dept_selected == '21';
        }
        
        $dept_desc = trim($this->master_prod_target_m->replacer_dept_prd($dept_selected));
              
        $overtime_summary = $this->master_prod_target_m->get_overtime_summary_all($date_selected, $dept_selected, $sect_selected, $subsect_selected);
        ...
        $objPHPExcel->getProperties()->setTitle("Report Overtime per Sub Sect");
        $objPHPExcel->getProperties()->setSubject("Report Overtime per Sub Sect");
        $objPHPExcel->getProperties()->setDescription("Report Overtime per Sub Sect");
        ...
        //SETUP EXCEL
        //TITLE
        $objPHPExcel->getActiveSheet()->setCellValue('A1', 'Report Overtime per Sub Section');
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('A3', 'Sect');
        if($sect_selected == '' || $sect_selected == 'ALL' || $sect_selected == null){
            $objPHPExcel->getActiveSheet()->setCellValue('B3', ': ALL');
        } else{
            $objPHPExcel->getActiveSheet()->setCellValue('B3', ': ' . $sect_selected);
        }
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('B4', 'Sub Section');
        ...
        foreach ($overtime_summary as $row) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue("B$e", $row->KD_SUB_SECTION);
            ...
            $actual_hour = $aortadb->query("EXEC zsp_get_actual_overtime_hour '$date_selected', '" . $row->KD_DEPT . "' , '" . $row->KD_SECTION . "' , '" . $row->KD_SUB_SECTION . "'")->row();
            ...
            $actual_amount = $aortadb->query("EXEC zsp_get_actual_overtime_amount '$date_selected', '" . $row->KD_DEPT . "' , '" . $row->KD_SECTION . "' , '" . $row->KD_SUB_SECTION . "'")->row();
        ...
        if ($sect_selected == '' || $sect_selected == null || $sect_selected == 'ALL'){
            $filename = "OT Report-" . trim($dept_desc) . "-ALL-ALL-" . trim($date_selected) . "-" . date("Y/m/d") . ".xlt";
        } else {
            if ($subsect_selected == '' || $subsect_selected == null || $subsect_selected == 'ALL'){
                $filename = "OT Report-" . trim($dept_desc) . "-" . trim($sect_selected). "-ALL-" . trim($date_selected) . "-" . date("Y/m/d") . ".xlt";
            } else {
                $filename = "OT Report-" . trim($dept_desc) . "-" . trim($sect_selected). "-" . trim($subsect_selected) . "-" . trim($date_selected) . "-" . date("Y/m/d") . ".xlt";
            }
        }
    ...
    public function plan_vs_actual($period = null, $dept = null, $section = null) {
        ...
        $data['news'] = $this->news_m->get_news();
        $data['content'] = 'aorta/report_plan_vs_actual_v';
        ...
        if($section == 'ALL'){
            ...
            $section = $this->overtime_m->get_top_section_overtime(trim($dept))->row()->KODE;
        }else{
            $x = 0;
        ...
        $data['role'] = $role;
        $data['npk'] = $npk;
        $data['data'] = $this->overtime_m->get_data_plan_actual($period, $dept, $section);
        $this->load->view($this->layout, $data);
    }
