...
class report_overtime_persect_c extends CI_Controller {
    ...
    function index($periode = null, $id_dept = null, $id_section = null) {
        $this->role_module_m->authorization('164');
        $data['title'] = 'Report Overtime Per Section';
        $data['app'] = $this->role_module_m->get_app();
        $data['module'] = $this->role_module_m->get_module();
        $data['function'] = $this->role_module_m->get_function();
        $data['sidebar'] = $this->role_module_m->side_bar(164);
        $data['news'] = $this->news_m->get_news();
        $data['content'] = 'aorta/report_overtime_per_sect_v';
        ...
        $all_section = $this->master_prod_target_m->get_section($id_dept);

        if ($id_dept == null){
            $id_dept = 21;
        }

        $data['overtime_quota'] = $this->master_prod_target_m->get_overtime_quota_by_section($selected_date, $id_dept, $id_section);
        $data['overtime_summary'] = $this->master_prod_target_m->get_overtime_summary_by_section($selected_date, $id_dept, $id_section);
        
        $data['all_section'] = $all_section;
    ...
    function print_report_overtime() {
        ...
        $overtime_summary = $this->master_prod_target_m->get_overtime_summary_by_section($date_selected, $dept_selected, $sect_selected);
        ...
        $objPHPExcel->getProperties()->setTitle("Report Overtime per Sect");
        $objPHPExcel->getProperties()->setSubject("Report Overtime per Sect");
        $objPHPExcel->getProperties()->setDescription("Report Overtime per Sect");
        ...
        //SETUP EXCEL
        //TITLE
        $objPHPExcel->getActiveSheet()->setCellValue('A1', 'Report Overtime per Section');
        ...
        foreach ($overtime_summary as $row) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue("B$e", $row->KD_SECTION);
            ...
            $actual_hour = $aortadb->query("EXEC zsp_get_actual_overtime_hour_section '$date_selected', '" . $row->KD_DEPT . "' , '" . $row->KD_SECTION . "'")->row();
            ...
            $actual_amount = $aortadb->query("EXEC zsp_get_actual_overtime_amount_section '$date_selected', '" . $row->KD_DEPT . "' , '" . $row->KD_SECTION . "'")->row();
        ...
        if($sect_selected == '' || $sect_selected == null || $sect_selected == 'ALL'){
            $filename = "OT Report-" . trim($dept_desc) . "-ALL-" . trim($date_selected) . "-" . date("Y/m/d") . ".xlt";
        } else {
            $filename = "OT Report-" . trim($dept_desc) . "-" . trim($sect_selected) . "-" . trim($date_selected) . "-" . date("Y/m/d") . ".xlt";
        }
