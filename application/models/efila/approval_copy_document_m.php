<?php
	...
	*/
	class approval_copy_document_m extends CI_Model
	{
		...
		private $table = 'TT_COPY_DOC';

		function get_copy_spv() {
			$efiladb = $this->load->database("efila", TRUE);
			$sect = $this->session->userdata("SECTION");
	    	$query = $efiladb->query("SELECT D.INT_ID_DOCUMENT, D.CHR_NO_DOC, D.CHR_DOCUMENT_NAME, D.INT_REVISION, C.CHR_CATEGORY_NAME, R.CHR_CREATED_BY, R.CHR_CREATED_DATE, D.CHR_DOC, R.INT_ID_COPY, R.INT_TYPE, R.INT_TOTAL FROM TM_DOCUMENT D JOIN TM_CATEGORY_DOC C ON D.INT_ID_CATEGORY = C.INT_ID_CATEGORY JOIN TT_COPY_DOC R ON R.INT_ID_DOCUMENT = D.INT_ID_DOCUMENT WHERE R.INT_APPROVED_SPV IS NULL AND R.INT_STATUS = 1 AND R.INT_ID_SECTION = $sect");
	    	return $query->result();
	    }
		...
		function get_copy_back($id) {
			$efiladb = $this->load->database("efila", TRUE);
			$query = $efiladb->query("SELECT * FROM TT_COPY_BACK WHERE INT_ID_COPY = $id");
	    	return $query->result();
	    }
		...
		function get_cat_name($id) {
			$efiladb = $this->load->database("efila", TRUE);			
			$query = $efiladb->query("SELECT C.CHR_CATEGORY_NAME FROM TT_COPY_DOC R JOIN TM_DOCUMENT D ON R.INT_ID_DOCUMENT = D.INT_ID_DOCUMENT JOIN TM_CATEGORY_DOC C ON D.INT_ID_CATEGORY = C.INT_ID_CATEGORY WHERE R.INT_ID_COPY = $id");
			return $query->result();
		}

		function get_npk($id) {
			$efiladb = $this->load->database("efila", TRUE);
			$query = $this->db->query("SELECT U.CHR_NPK FROM TT_COPY_DOC R JOIN TM_USER U ON R.CHR_CREATED_BY = U.CHR_USERNAME WHERE R.INT_ID_COPY = $id");
			return $query->result();
		}
	}
?>