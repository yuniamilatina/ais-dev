...
class setup_chute_m extends CI_Model
{
    ...
    function updateSetupChute($id_cavity, $data)
    {
        $this->db->where('INT_CAVITY', $id_cavity);
        $this->db->update($this->tabel, $data);
    }
    ...
    function get_setup_chute_open($work_center)
    {
        ...
        $sql = "SELECT a.INT_ID, a.INT_SEQUENCE, a.CHR_WORK_CENTER, a.CHR_PART_NO, a.CHR_PRD_ORDER_NO, a.INT_LOT_SIZE, a.INT_QTY_PER_BOX, 
                        ...
                        a.CHR_PRD_ORDER_NO_REFF, a.INT_RECOVERY, a.INT_CAVITY
    ...
    function get_setup_chute_close($work_center)
    {
        ...
        sc.CHR_PRD_ORDER_NO_REFF, sc.INT_RECOVERY, b.CHR_FLAG, sc.INT_CAVITY
        FROM $this->tabel sc 
    ...
    function get_setup_chute_uncomplete($work_center)
    {
        $sql = "SELECT TOP 50 a.INT_ID, a.INT_SEQUENCE, a.CHR_WORK_CENTER, a.CHR_PART_NO, a.CHR_PRD_ORDER_NO, a.INT_LOT_SIZE, 
            ...
            p.CHR_MATRIX_DANDORI, a.INT_FLG_RECOVERY, a.INT_QTY_PCS_ACTUAL, a.CHR_PRD_ORDER_NO_REFF, a.INT_RECOVERY, a.INT_CAVITY
            FROM $this->tabel a
            ...
            ORDER BY substring(a.CHR_PRD_ORDER_NO,8,8) DESC";
    ...
    {
        $sql = "SELECT INT_ID, INT_SEQUENCE, CHR_WORK_CENTER, CHR_PART_NO, CHR_PRD_ORDER_NO, INT_LOT_SIZE, INT_QTY_PER_BOX, INT_QTY_PCS, CHR_DATE, CHR_NOTES, CHR_STATUS_BOM, INT_FLG_ADJUST_FINISH, CHR_NOTES_UNCOMPLETE, INT_FLG_PRD, INT_CAVITY FROM $this->tabel 
            WHERE INT_ID = '$id'";
    ...
    function update_sequence_bigger_cavity($work_center, $old_seq, $new_seq, $date, $time, $user)
    {
        $datenow = date('Ymd');
        $timenow = date('his');
    ...
    function get_last_id_chute($prod_no)
    {
        $sql = "SELECT TOP 1 INT_ID FROM PRD.TT_SETUP_CHUTE
            WHERE CHR_PRD_ORDER_NO = '$prod_no' ORDER BY INT_ID DESC";
    ...
    function updateSetupChuteNotUse($work_center, $prod_order_no)
    {
        $datenow = date('Ymd');
        $timenow = date('His');
        ...
        $data_sequence = $this->db->query("SELECT * FROM $this->tabel WHERE INT_STATUS_UNCOMPLETE = 1 AND
                CHR_WORK_CENTER = '$work_center' AND INT_FLG_PRD <> '2' AND INT_FLG_DEL = '0' ORDER BY INT_SEQUENCE, INT_ID ASC")->result();
    ...
    function update_dandori_setup_chute($work_center, $date, $prod_order_no)
    {
        ...
        // $this->db->where('INT_FLG_PRD', 1);//--20220612
        $this->db->where('INT_FLG_DEL', 0);
        $this->db->update($this->tabel, $data);
    ...
    //for cavity 20220612
    function updateDandoriSetupChute($work_center, $id_setup_chute, $prod_order_no)
    {
        $datenow = date('Ymd');
        $timenow = date('His');

        $chute_dandory_uncomplete = $this->db->query("SELECT * FROM $this->tabel 
        WHERE CHR_WORK_CENTER = '$work_center' AND INT_FLG_PRD = 1 AND INT_SEQUENCE = 0 AND INT_FLG_DEL = 0
        AND INT_CAVITY NOT IN (SELECT INT_CAVITY FROM $this->tabel  WHERE INT_ID = $id_setup_chute)");
        ...
        if ($chute_dandory_uncomplete->num_rows() > 0) {
            if ($chute_dandory_uncomplete->row()->INT_STATUS_UNCOMPLETE == 1) {
                ...
                if($chute_dandory_uncomplete->row()->INT_CAVITY != NULL){ //add by toro 202208

                    $data_uncompleted = array(
                        'CHR_STATUS_INLINE' => 'Uncompleted',
                        'CHR_INLINE_DATE' => $datenow,
                        'CHR_INLINE_TIME' => $timenow,
                        'INT_STATUS_UNCOMPLETE' => 1,
                        'INT_FLG_PRD' => 2
                    ...
                    $this->db->where('INT_CAVITY', $chute_dandory_uncomplete->row()->INT_CAVITY);
                    $this->db->where('INT_FLG_DEL', 0);
                    $this->db->where('INT_STATUS_UNCOMPLETE', 1);
                    $this->db->where('WORK_CENTER', $work_center); 
                    $this->db->update($this->tabel, $data_uncompleted);
                }
                ...
                if($chute_dandory_uncomplete->row()->INT_CAVITY != NULL){ //add by toro 202208
                    ...
                    $this->db->where('INT_CAVITY', $chute_dandory_uncomplete->row()->INT_CAVITY);
                    $this->db->where('INT_FLG_DEL', 0);
                    $this->db->where('INT_STATUS_UNCOMPLETE', 0);
                    $this->db->where('WORK_CENTER', $work_center); 
                    $this->db->update($this->tabel, $data_completed);
                }
        ...
        $this->db->where('INT_ID', $id_setup_chute); //===== Update by ANU 20190228
        $this->db->update($this->tabel, $data);

        if ($phantom == 0) {  //===== Update after check phantom ELINA status --- by ANU 20211216 =====//
            //flag on progress production
            //change CHR_PRD_ORDER_NO = $prod_order_no
            $this->db->query("UPDATE $this->elina_h SET CHR_FLAG = '6' WHERE CHR_FLAG = '5' AND CHR_PRD_ORDER_NO = '$prod_order_no'");
        } //===== Update after check phantom ELINA status --- by ANU 20211216 =====//
        ...
        $y = 1;
        foreach ($data_sequence as $isi) {
            $this->db->query("UPDATE $this->tabel SET INT_SEQUENCE = $y WHERE INT_ID = '$isi->INT_ID' AND INT_FLG_DEL = 0");
            $y++;
        }
    }
    ...
    {
        $sql = "SELECT TOP 1  sc.INT_CAVITY, sc.CHR_PRD_ORDER_NO, sc.CHR_PART_NO, sc.CHR_BACK_NO, sc.INT_LOT_SIZE_ACTUAL, sc.INT_LOT_SIZE, sc.INT_QTY_PER_BOX, sc.INT_QTY_PCS, sc.INT_QTY_PCS_ACTUAL, sc.INT_FLG_RECOVERY, sc.INT_RECOVERY, sc.CHR_PRD_ORDER_NO_REFF, p.CHR_PART_NAME
            FROM $this->tabel sc
    ...
    function update_actual_lot($work_center, $prod_order_no)
    {
        ...
        $this->db->query("UPDATE $this->tabel SET INT_LOT_SIZE_ACTUAL = INT_LOT_SIZE_ACTUAL - 1, INT_QTY_PCS_ACTUAL = INT_QTY_PCS_ACTUAL + INT_QTY_PER_BOX,
            INT_STATUS_UNCOMPLETE = 1, CHR_STATUS_INLINE = 'OnProcess', CHR_INLINE_DATE = '$datenow', CHR_INLINE_TIME = '$timenow'
        ...
        if ($data_sequence->INT_LOT_SIZE_ACTUAL == 0) {
            $this->db->query("UPDATE $this->tabel SET INT_FLG_PRD = '2', INT_STATUS_UNCOMPLETE = 0, CHR_STATUS_INLINE = 'Completed', CHR_INLINE_DATE = '$datenow', CHR_INLINE_TIME = '$timenow'
            WHERE CHR_WORK_CENTER = '$work_center' AND INT_FLG_DEL = '0' AND INT_SEQUENCE = 0 AND CHR_PRD_ORDER_NO = '$prod_order_no'");
    ...
    //looper 202206
    function getSetupChutebyWorkCenter($work_center)
    {
        $sql = "SELECT SC.INT_ID, SC.INT_SEQUENCE, SC.CHR_PRD_ORDER_NO, LEFT(RTRIM(SC.CHR_PRD_ORDER_NO_REFF),19) AS CHR_PRD_ORDER_NO_REFF, SC.CHR_WORK_CENTER, SC.CHR_PART_NO, SC.CHR_BACK_NO, 
            ...
            INNER JOIN TM_PARTS P ON P.CHR_PART_NO = SC.CHR_PART_NO
            WHERE SC.CHR_WORK_CENTER = '$work_center' AND SC.INT_FLG_PRD = '1' AND SC.INT_FLG_DEL = '0' 
            AND INT_CAVITY IN (
			SELECT TOP 1 INT_CAVITY FROM PRD.TT_SETUP_CHUTE WHERE CHR_WORK_CENTER = '$work_center' AND INT_FLG_PRD = '1' AND INT_FLG_DEL = '0' AND INT_SEQUENCE = 1
			)
            ORDER BY SC.INT_SEQUENCE ASC";
