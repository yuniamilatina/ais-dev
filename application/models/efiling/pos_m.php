...
class pos_m extends CI_Model {
	...
	function get_pos_per_sub_section() {
        $query = $this->db->query("select a.CHR_SECTION_DESC, b.CHR_SUB_SECTION, c.INT_POS_NUMBER, c.CHR_POS_TITLE, c.INT_ID_POS
			from TM_SECTION a, TM_SUB_SECTION b, TM_EFILING_POS c
    ...
    function get_number_pos($id) {
        $query = $this->db->query("select INT_POS_NUMBER from TM_EFILING_POS where INT_ID_POS = '" . $id . "'")->row_array();
        $pos = $query['INT_POS_NUMBER'];
        return $pos;
    }

    function get_title_pos($id) {
        $query = $this->db->query("select CHR_POS_TITLE from TM_EFILING_POS where INT_ID_POS = '" . $id . "'")->row_array();
    ...
    function generate_id_pos() {
        return $this->db->query('select max(INT_ID_POS) as a from TM_EFILING_POS')->row()->a + 1;
    }
