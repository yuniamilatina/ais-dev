...
class pos_doc_m extends CI_Model {

     private $tabel = 'TT_EFILING_POS_DOC';
    ...
    function get_name_doc($id) {
        $query = $this->db->query("select CHR_DOC_TITLE from TM_EFILING_DOC where INT_ID_DOC = '" . $id . "'")->row_array();
    ...
    function generate_id_doc() {
        return $this->db->query('select max(INT_ID_POS_DOC) as a from TT_EFILING_POS_DOC')->row()->a + 1;
    }
