...
class Nolocking_m extends CI_Model {
	...
	{
		if (empty($data)):
			if (empty($_GET['CHR_AREA'])) return array('status' => 'error', 'message' => 'Field area harus di isi.');
			$query = (!empty($_GET['CHR_LKB_NO']))? array('where' => "CHR_AREA = '".$this->get_id($this->input->get('CHR_AREA'))."' AND CHR_LKB_NO = '".$this->input->get('CHR_LKB_NO')."' AND CHR_VALIDATE_MSU IS NULL", 'order_by' => 'INT_NUMBER DESC') : array('where' => "CHR_AREA = '".$this->get_id($this->input->get('CHR_AREA'))."' AND CHR_VALIDATE_MSU IS NULL AND CHR_LKB_NO IS NULL AND CHR_VALIDATE_LEADER IS NULL", 'order_by' => 'INT_NUMBER DESC');
			$check = $this->execute($query,'tt_ng_record_h');
				...
				// get validation data
				$result = @$this->execute(array('where' => "INT_NUMBER IN (".$int_number.") AND CHR_AREA = '".$this->get_id($this->input->get('CHR_AREA'))."' AND CHR_DEL_FLG IS NULL"),'tt_ng_record_l');
				if (@$result){
					foreach ($result as $key => $value) {
			...
			$lkb_no = $this->execute(array('select' => 'CASE WHEN COUNT(CHR_LKB_NUM) > 0 THEN MAX(CAST(CHR_LKB_NUM AS INT))+1 ELSE 1 END AS CHR_LKB_NUM' , 'where' => 'CHR_LKB_NUM IS NOT NULL AND CHR_LKB_DATE ="'. date('ym').'"', 'limit'=>1,'order_by' =>'CHR_LKB_NUM DESC' ),'tt_ng_record_h',true)->CHR_LKB_NUM;
    ...
    /**********************************************************CHANGE OR NOT USED**************************************************************/
	/*public function generate_reprint()
	{
		$check = $this->execute(array('where' => "CHR_LKB_NO = '".$this->get_id($this->input->post('CHR_LKB_NO'))."'", 'limit' => 1, 'order_by' => 'INT_NUMBER DESC'),'tt_ng_record_h',true);
		if ($check):
				// get validation data
				$result = $this->execute(array('where' => "CHR_AREA = '".$check->CHR_AREA."' AND CHR_LKB_NO = '".$this->get_id($this->input->post('CHR_LKB_NO'))."' AND CHR_VALIDATE_LEADER IS NOT NULL"),'tt_ng_record_l');
				return $result;
			endif;
	}*/
}