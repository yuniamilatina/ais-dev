        ...
        <div class="row">
            <div class="col-md-12">
                <div class="grid">
                    ...
                    <div class="grid-body">
                    <?php
                    echo form_open_multipart('part/part_customer_c/reupload_part_customer_wi', 'class="form-horizontal"');
                    ?>
                   
                   <div class="form-group">
                        <label class="col-sm-3 control-label">Part No (Customer)</label>
                        <div class="col-sm-2">
                            <select  name="CHR_CUS_PART_NO" id="e1" onchange="get_data_part_cust();get_data_cust();" class="form-control">
                                <?php foreach ($data_part_no_cust as $row_cust_no) { 
                                    if (trim($detail_wi->CHR_CUS_PART_NO) == trim($row_cust_no->CHR_CUS_PART_NO)){ ?>
                    ...
                    <div class="form-group">
                        <label class="col-sm-3 control-label">Part No (Aisin)</label>
                        <div class="col-sm-6" >
                            <select class="form-control" name="CHR_PART_NO" id="part_no_aisin" required style="width:200px;">
                                <?php foreach ($data_part_no_aisin as $row) { 
                                    if(trim($row->CHR_PART_NO) == $detail_wi->CHR_PART_NO) {?>
                                        <option selected value="<?php echo $row->CHR_PART_NO; ?>"><?php echo trim($row->CHR_PART_NO); ?></option>
                                    <?php }else{ ?>
                        ...
                        <div class="col-sm-6" >
                            <select class="form-control" name="CHR_CUS_NO" id="cus_no" required style="width:300px;">
                                <?php foreach ($data_cust as $row) { 
                                    if (trim($row->CHR_CUST_NO) == $detail_wi->CHR_CUS_NO) { ?>
                                        <option selected value="<?php echo $row->CHR_CUST_NO; ?>"><?php echo trim($row->CHR_CUST_NAME); ?></option>
                                    <?php }else{ ?>
                                        <option value="<?php echo $row->CHR_CUST_NO; ?>"><?php echo trim($row->CHR_CUST_NAME); ?></option>
                                <?php }
                                }
                                ?> 
                            </select>
                    ...
                    <input type="hidden" name="INT_ID" value="<?php echo $detail_wi->INT_ID; ?>">
                    <input type="hidden" name="CHR_CUS_PART_NO_OLD" value="<?php echo $detail_wi->CHR_CUS_PART_NO; ?>">
