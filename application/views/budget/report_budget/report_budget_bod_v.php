                    ...
                    data: [
                        {
                            ...
                            dataPoints: [
<?php
$count_budget = count($data_report_bod);
$tot_plan_plant = 0;
$row_plan = 1;
if($act_periode < $periode_smt2){
    //BEFORE REVISE BUDGET
    foreach ($data_report_bod as $bgt) {
        if ($row_plan == $count_budget) {
            echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($bgt->TOT_PLAN / 1000000,2) . ', indexLabel:"' . round($bgt->TOT_PLAN / 1000000000,2) . ' bio"}';
        } else {
            echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($bgt->TOT_PLAN / 1000000,2) . ', indexLabel:"' . round($bgt->TOT_PLAN / 1000000000,2) . ' bio"},';
        }
        $tot_plan_plant = $tot_plan_plant + $bgt->TOT_PLAN;
        $row_plan++;
    }
} else {
    //AFTER REVISE BUDGET
    foreach ($data_report_bod as $bgt) {
        $group = $bgt->CHR_KODE_GROUP;
        $kd_group = '';
        if($group == 'PRODUCTION'){
            $kd_group = '001';
        ...
        $plan = $this->report_budget_m->get_new_budget_detail_gm($fiscal_start, $fiscal_start, $fiscal_end, $new_bgt_type, $kd_group);
        ...
        if ($row_plan == $count_budget) {
            echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($tot_plan / 1000000,2) . ', indexLabel:"' . round($tot_plan / 1000000000,2) . ' bio"}';
        } else {
            echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($tot_plan / 1000000,2) . ', indexLabel:"' . round($tot_plan / 1000000000,2) . ' bio"},';
        }
        $tot_plan_plant = $tot_plan_plant + $tot_plan;
        $row_plan++;
    }
}
?>
...
foreach ($data_report_bod as $bgt) {
    ...
    $revise = $this->report_budget_m->get_new_budget_detail_rev_gm($fiscal_start, $fiscal_start, $fiscal_end, $new_bgt_type, $kd_group);
    if($act_periode < $periode_smt2){
        //BEFORE REVISE BUDGET
        $tot_rev = 0;
    } else {
    ...
    if ($row_rev == $count_budget) {
        echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($tot_rev / 1000000,2) . ', indexLabel:"' . round($tot_rev / 1000000000,2) . ' bio"}';
    } else {
        echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($tot_rev / 1000000,2) . ', indexLabel:"' . round($tot_rev / 1000000000,2) . ' bio"},';
    }
//    if ($row_rev == $count_budget) {
...
//        echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($bgt->TOT_PLAN / 1000000,2) . ', indexLabel:"' . round($bgt->TOT_PLAN / 1000000000,2) . ' bio"},';
//    }
    $tot_rev_plant = $tot_rev_plant + $tot_rev;
    $row_rev++;
}
?>
                        ]
...
foreach ($data_report_bod as $bgt) {
    if($act_periode < $periode_smt2){
        ...
        if ($row_lim == $count_budget) {
            echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($bgt->TOT_LIMIT / 1000000,2) . ', indexLabel:"' . round($bgt->TOT_LIMIT / 1000000000,2) . ' bio"}';
        } else {
            echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($bgt->TOT_LIMIT / 1000000,2) . ', indexLabel:"' . round($bgt->TOT_LIMIT / 1000000000,2) . ' bio"},';
        }
        $tot_limit_plant = $tot_limit_plant + $bgt->TOT_LIMIT;
        $row_lim++;
    } else {    
        ...
        $tot_rev = $revise->PBLN04 + $revise->PBLN05 + $revise->PBLN06 + $revise->PBLN07 + $revise->PBLN08 + $revise->PBLN09 + $revise->PBLN10 + $revise->PBLN11 + $revise->PBLN12 + $revise->PBLN13 + $revise->PBLN14 + $revise->PBLN15;
        if ($row_lim == $count_budget) {
            echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round(($tot_rev*(70/100)) / 1000000,2) . ', indexLabel:"' . round(($tot_rev*(70/100)) / 1000000000,2) . ' bio"}';
        } else {
            echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round(($tot_rev*(70/100)) / 1000000,2) . ', indexLabel:"' . round(($tot_rev*(70/100)) / 1000000000,2) . ' bio"},';
        }
        $tot_limit_plant = $tot_limit_plant + ($tot_rev*(70/100));
        $row_lim++;        
    }
}
?> 
...
$row_act = 1;
if($act_periode < $periode_smt2){
    //ACTUAL PR BEFORE REVISE
    foreach ($data_report_bod as $bgt) {    
        if ($row_act == $count_budget) {        
            echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($bgt->TOT_ACTUAL / 1000000,2) . ', indexLabel:"' . round($bgt->TOT_ACTUAL / 1000000000,2) . ' bio"}';
        } else {
            echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($bgt->TOT_ACTUAL / 1000000,2) . ', indexLabel:"' . round($bgt->TOT_ACTUAL / 1000000000,2) . ' bio"},';
        }

        $tot_act_plant = $tot_act_plant + $bgt->TOT_ACTUAL;
        $row_act++;
    }
} else {
    //ACTUAL PR AFTER REVISE
    foreach ($data_report_bod as $bgt) { 
        $group = $bgt->CHR_KODE_GROUP;
        $kd_group = '';
        if($group == 'PRODUCTION'){
        ...
        $pr = $this->report_budget_m->get_new_actual_real_gm($fiscal_start, $fiscal_start, $fiscal_end, $new_bgt_type, $kd_group);
        ...
        if ($row_act == $count_budget) {        
            echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($tot_pr / 1000000,2) . ', indexLabel:"' . round($tot_pr / 1000000000,2) . ' bio"}';
        } else {
            echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($tot_pr / 1000000,2) . ', indexLabel:"' . round($tot_pr / 1000000000,2) . ' bio"},';
        }

        $tot_act_plant = $tot_act_plant + $tot_pr;
        $row_act++;
    }
}
?>
...
foreach ($data_report_bod as $bgt) {
    if ($bgt_type == 'CAPEX'){
        $data_gr = $bgt_aii->query("EXEC zsp_get_gr_capex_chart_group '$fiscal_start', '$fiscal_end' , '".$bgt->CHR_KODE_GROUP."', ''");
    } else {
        if($bgt_type == 'ALL'){
            $new_bgt_type = '';
        } else {
        ...
        $data_gr = $bgt_aii->query("EXEC zsp_get_gr_expense_chart_group '$fiscal_start', '$fiscal_end' , '$new_bgt_type' , '".$bgt->CHR_KODE_GROUP."', ''");
    }
    ...
    if ($row_act == $count_budget) {        
        echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($tot_gr / 1000000,2) . ', indexLabel:"' . round($tot_gr/ 1000000000,2) . ' bio"}';
    } else {
        echo '{label: "' . $bgt->CHR_KODE_GROUP . '", y:' . round($tot_gr / 1000000,2) . ', indexLabel:"' . round($tot_gr / 1000000000,2) . ' bio"},';
    }
    $tot_gr_plant = $tot_gr_plant + $tot_gr;
    $row_act++;
}
?>
                        ]
...
if($act_periode < $periode_smt2){
    ...
    foreach ($data_report_gm as $bgt) {
        if ($row_plan == $count_budget) {
            echo '{label: "' . $bgt->CHR_KODE_DEPARTMENT . '", y:' . round($bgt->TOT_PLAN / 1000000,2) . ', indexLabel:"' . round($bgt->TOT_PLAN / 1000000000,2) . '"}';
        } else {
            echo '{label: "' . $bgt->CHR_KODE_DEPARTMENT . '", y:' . round($bgt->TOT_PLAN / 1000000,2) . ', indexLabel:"' . round($bgt->TOT_PLAN / 1000000000,2) . '"},';
        }
        $row_plan++;
...
if($act_periode < $periode_smt2){
    ...
    foreach ($data_report_gm as $bgt) {
        if ($row_lim == $count_budget) {
            echo '{label: "' . $bgt->CHR_KODE_DEPARTMENT . '", y:' . round($bgt->TOT_LIMIT / 1000000,2) . ', indexLabel:"' . round($bgt->TOT_LIMIT / 1000000000,2) . '"}';
        } else {
            echo '{label: "' . $bgt->CHR_KODE_DEPARTMENT . '", y:' . round($bgt->TOT_LIMIT / 1000000,2) . ', indexLabel:"' . round($bgt->TOT_LIMIT / 1000000000,2) . '"},';
        }
        $row_lim++;
...
if($act_periode < $periode_smt2){
    ...
    foreach ($data_report_gm as $bgt) {
        if ($row_act == $count_budget) {        
            echo '{label: "' . $bgt->CHR_KODE_DEPARTMENT . '", y:' . round($bgt->TOT_ACTUAL / 1000000,2) . ', indexLabel:"' . round($bgt->TOT_ACTUAL / 1000000000,2) . '"}';
        } else {
            echo '{label: "' . $bgt->CHR_KODE_DEPARTMENT . '", y:' . round($bgt->TOT_ACTUAL / 1000000,2) . ', indexLabel:"' . round($bgt->TOT_ACTUAL / 1000000000,2) . '"},';
        }
        $row_act++;
...
//                            }
            ...
            chart.render(); //CHART PER GROUP
            chart_2.render(); //CHART PER DEPT
        }
</script>
                <div class="grid">
                    <div class="grid-header">
                        <i class="fa fa-bar-chart-o"></i>
                        <span class="grid-title"><strong>REPORT BUDGET BY GROUP - <?php echo $bgt_type; ?></strong></span>
                    ...
                    <div class="grid-body">
                                ...
                                <tr>
                                    ...
                                    <td width="20%">
                                        <select name="CHR_FISCAL_YEAR" class="form-control" id="tahun" onChange="document.location.href = this.options[this.selectedIndex].value;">
                                            <?php foreach ($data_fiscal as $data) { ?>
                                                <option value="<?PHP echo site_url('budget/report_budget_c/report_budget_for_bod/' . $data->CHR_FISCAL_YEAR_START . '/' . $bgt_type . '/' . $kode_group); ?>" <?php
                                    ...
                                    <td width="20%">
                                        <select name="CHR_GROUP" class="form-control" id="dept_name" onchange="document.location.href = this.options[this.selectedIndex].value;">    
                                            <option value='<?php echo site_url('budget/report_budget_c/report_budget_for_bod/' . $fiscal_start . '/' . $bgt_type . '/ALL'); ?>'>ALL</option>
                                            <?php foreach ($list_group as $group) { ?>
                                                <option value="<?php echo site_url('budget/report_budget_c/report_budget_for_bod/' . $fiscal_start . '/' . $bgt_type . '/' . trim($group->CHR_KODE_GROUP)); ?>" <?php
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select name="CHR_BUDGET_TYPE" class="form-control" id="budget_type" onchange="document.location.href = this.options[this.selectedIndex].value;">
                                            ...
                                            <option value="<?php echo site_url('budget/report_budget_c/report_budget_for_bod/' . $fiscal_start . '/ALL/' . $kode_group); ?>" <?php if($bgt_type == 'ALL') { echo 'selected';}?>> ALL EXPENSE </option>
                                            <?php foreach ($list_budget_type as $bgt) { ?>
                                                <option value="<?php echo site_url('budget/report_budget_c/report_budget_for_bod/' . $fiscal_start . '/' . trim($bgt->CHR_BUDGET_TYPE) . '/' . $kode_group); ?>" <?php
                        ...
                        &nbsp;   
                        <div align="center">
                            <strong>Plan Ori</strong> : <span style="font-size: small; font-weight: bold;" class="label label-primary"><?php echo number_format($tot_plan_plant, 0, ',', '.'); ?></span> &nbsp;&nbsp;
                            <strong>Revise</strong> : <span style="font-size: small; font-weight: bold;" class="label label-danger"><?php echo number_format($tot_rev_plant, 0, ',', '.'); ?></span> &nbsp;&nbsp;
                            <strong>Limit</strong> : <span style="font-size: small; font-weight: bold;" class="label label-success"><?php echo number_format($tot_limit_plant, 0, ',', '.'); ?></span> &nbsp;&nbsp;
                            <strong>Act PR</strong> : <span style="font-size: small; font-weight: bold;" class="label label-info"><?php echo number_format($tot_act_plant, 0, ',', '.'); ?></span> &nbsp;&nbsp;
                            <strong>Act GR</strong> : <span style="font-size: small; font-weight: bold; background-color: #A692BD;" class="label label-info"><?php echo number_format($tot_gr_plant, 0, ',', '.'); ?></span> &nbsp;&nbsp;
                        </div>
                    </div>                    
                </div>
            </div>
            ...
            <div class="col-md-12">
                <div class="grid">
                    <div class="grid-header">
                        ...
                        <span class="grid-title"><strong>REPORT DETAIL BUDGET BY GROUP - <?php echo $bgt_type; ?></strong></span>
                        <div class="pull-right grid-tools">
                            <a data-widget="collapse" title="Collapse"><i class="fa fa-chevron-up"></i></a>
                        </div>                        
                    </div>
                        ...
                        <table id="example1" class="table table-condensed table-bordered table-striped table-hover display" cellspacing="0" width="100%" style=" font-size: 11px;">
                            <thead>
                                <tr style="font-weight:bold;">
                                    ...
                                    <td rowspan="2" align='center' style="vertical-align: middle;">Group</td>
                            ...
                            <tbody>
                                ...
                                $i = 1;
                                foreach ($report_per_group as $isi) {
                                        if ($bgt_type == 'CAPEX'){
                                            $GRBLN = $bgt_aii->query("EXEC zsp_get_gr_capex_by_group '$fiscal_start', '$fiscal_end' , '" . $isi->CHR_KODE_GROUP . "', ''");
                                        } else {
                                            $GRBLN = $bgt_aii->query("EXEC zsp_get_gr_expense_by_group '$fiscal_start', '$fiscal_end' , '$new_bgt_type' , '" . $isi->CHR_KODE_GROUP . "', ''");
                                        }
                                        ...
                                        echo "<td align='center'>" . $isi->CHR_KODE_GROUP . "</td>";
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="grid">
                    <div class="grid-header">
                        <i class="fa fa-book"></i>
                        <span class="grid-title"><strong>REPORT DETAIL BUDGET BY DEPT - <?php echo $bgt_type; ?></strong></span>
                        <div class="pull-right grid-tools">
                            <a data-widget="collapse" title="Collapse"><i class="fa fa-chevron-up"></i></a>
                        </div>
                    </div>
