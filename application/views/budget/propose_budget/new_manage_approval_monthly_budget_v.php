            ...
            <div class="col-md-12">
                <div class="grid">
                    <div class="grid-header">
                        ...
                        <span class="grid-title"><strong>APPROVAL PROPOSE BUDGET FOR <?php echo strtoupper(date("F", mktime(null, null, null, $month))) . ' ' . substr($year_month,0,4) . ' - ' . $kode_dept; ?></strong></span>
                                    ...
                                    <td width="15%">
                                        <select name="CHR_FISCAL" class="form-control" id="fiscal" onChange="document.location.href = this.options[this.selectedIndex].value;">
                                            <?php foreach ($all_fiscal as $data) { ?>
                                                <option value="<?PHP echo site_url('budget/new_propose_budget_c/manage_approval_monthly_budget/0/' . $data->CHR_FISCAL_YEAR_START . '/' . $year_month . '/' . $kode_dept); ?>" <?php
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select class="form-control" id="month" onChange="document.location.href = this.options[this.selectedIndex].value;">
                                            <?php for($i = 0; $i < 12; $i++) { ?>
                                                <option value="<?PHP echo site_url('budget/new_propose_budget_c/manage_approval_monthly_budget/0/' . $fiscal_start . '/' . $all_month[$i][0] . '/' . $kode_dept); ?>"
                                            <?php 
                                    ...
                                    <td>
                                       <select name="CHR_DEPT" class="form-control" id="dept" onchange="document.location.href = this.options[this.selectedIndex].value;">
                                            ...
                                            <?php foreach ($all_dept as $dept) { ?>
                                                <option value="<?php echo site_url('budget/new_propose_budget_c/manage_approval_monthly_budget/0/' . $fiscal_start . '/' . $year_month . '/' . trim($dept->CHR_KODE_DEPARTMENT)); ?>" <?php
                            ...
                            <table style="font-size:12px;" id="dataTable" class="table table-condensed table-hover display" cellspacing="0" width="100%">
                                <thead>
                                    <tr>
                                        ...
                                        <td align="center"><strong>No Propose</strong></td>
                                        <td align="center"><strong>Year</strong></td>
                                        <td align="center"><strong>Month</strong></td>
                                        <td align="center"><strong>Department</strong></td>
                                        <?php if($role == '1' || $role == '2' || $role == '4' || $role == '3'){ ?>
                                            <td align="center"><strong>Approval Process</strong></td>
                                        <?php } ?>
                                        ...
                                        if($all_propose != NULL){
                                            ...
                                            foreach ($all_propose as $prop) {
                                                ...
                                                if($role == '1' || $role == '2'){
                                                    ...
                                                    } else if($prop->CHR_FLG_SWITCH == '3'){
                                                        echo "<td align='center' style='background-color: #5CB85C; color: white;'><strong>COMPLETED</strong></td>";
                                                    } else {
                                                        echo "<td align='center' style='background-color: #5BC0DE; color: white;'><strong>OUTSTANDING</strong></td>";
                                                    }
                                                } else if($role == '4'){
                                                    if($prop->CHR_FLG_SWITCH == 1){
                                                        echo "<td align='center' style='background-color: #F0AD4E; color: white;'><strong>WAITING</strong></td>";
                                                    } else if($prop->CHR_FLG_SWITCH >= 2){
                                                        echo "<td align='center' style='background-color: #5CB85C; color: white;'><strong>APPROVED</strong></td>";
                                                    }
                                                } else if($role == '3'){
                                                    if($prop->CHR_FLG_SWITCH == 2){
                                                        echo "<td align='center' style='background-color: #F0AD4E; color: white;'><strong>WAITING</strong></td>";
                                                    } else if($prop->CHR_FLG_SWITCH >= 3){
                                                        echo "<td align='center' style='background-color: #5CB85C; color: white;'><strong>APPROVED</strong></td>";
                                                    }
                                                }
                                                echo "<td align='center'><a href='" . base_url('index.php/budget/new_propose_budget_c/approval_monthly_budget') . "/0/" . str_replace('/','<',trim($prop->CHR_NO_PROPOSE)) . "' class='label label-primary' data-placement='left' title='View'><span class='fa fa-search'></span></a></td>";
                                                echo "</tr>";                                            
                                                $no++;
                                            }
                                        } else {
