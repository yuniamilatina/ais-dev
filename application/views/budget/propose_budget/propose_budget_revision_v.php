...
<script>
    ...
    function initDatatables2() {
        ...
        function fnFormatDetails(oTable, nTr) {
            ...
            var budget_type = aData[13];
            var no_budget = aData[3];
            var fiscal_start = aData[14];
            var fiscal_end = aData[15];
            ...
            $.ajax({
                ...
                type: "POST",
                dataType: 'json',
                url: "<?php echo site_url('budget/propose_budget_c/get_detail_budget_by_no'); ?>",
                ...
                <div class="grid">
                    <div class="grid-header">
                        ...
                        <div class="pull-right">
                            <a href="<?php echo base_url('index.php/budget/propose_budget_c/propose_unbudget') ?>"  class="btn btn-primary" data-placement="left" title="Propose Unbudget">Create Unbudget</a>
                        </div>
                    </div>
                    <div class="grid-body">
                        <div class="pull">
                                    ...
                                    <td width="20%">
                                        <select name="CHR_FISCAL" class="form-control" id="tahun" onChange="document.location.href = this.options[this.selectedIndex].value;">
                                            <?php foreach ($all_fiscal as $data) { ?>
                                                <option value="<?php echo site_url('budget/propose_budget_c/propose_budget_revision/0/' . $data->CHR_FISCAL_YEAR_START . '/CAPEX/ALL'); ?>" <?php
                                    ...
                                    <td width="20%">
                                        ...
                                        <?php } ?>
                                            <?php foreach ($all_dept as $dept) { ?>
                                                <option value="<?php echo site_url('budget/propose_budget_c/propose_budget_revision/0/' . $fiscal_start . '/' . $budget_type . '/' . $dept->INT_ID_DEPT); ?>" 
                                                <?php
                                    ...
                                    <td width="20%">
                                        <select name="CHR_BUDGET_TYPE" class="form-control" id="budget_type" onchange="document.location.href = this.options[this.selectedIndex].value;">
                                            <?php foreach ($all_bgt_type as $bgt) { ?>
                                                <option value="<?php echo site_url('budget/propose_budget_c/propose_budget_revision/0/' . $fiscal_start . '/' . trim($bgt->CHR_BUDGET_TYPE) . '/' . $kd_dept); ?>" 
                                                <?php
                        ...
                        <div>&nbsp;</div>
                        <div>
                            <table style="font-size:12px;" id="example" class="table table-striped table-condensed table-hover display" cellspacing="0" width="100%">
                                <thead>
                                    <tr>
                                        ...
                                        <td align="center"><strong>Original Amount (1FY)</strong></td>
                                        <td align="center"><strong>Limit Amount (1FY)</strong></td>
                                        <td align="center"><strong>On Propose (1FY)</strong></td>
                                        <!--<td align="center"><strong>Active</strong></td>-->
                                        <td align="center"><strong>Change Amount</strong></td>
                                        <td align="center"><strong>Reschedule</strong></td>
                                        <td align="center"><strong>Status</strong></td>
                                ...
                                <tbody>
                                    ...
                                    foreach ($list_master_budget as $list_bgt) {
                                        ...
                                        echo "<td>" . $desc . "</td>";
                                        $total_plan = $list_bgt->MON_BLN01 + $list_bgt->MON_BLN02 + $list_bgt->MON_BLN03 + $list_bgt->MON_BLN04 +
                                                $list_bgt->MON_BLN05 + $list_bgt->MON_BLN06 + $list_bgt->MON_BLN07 + $list_bgt->MON_BLN08 +
                                                $list_bgt->MON_BLN09 + $list_bgt->MON_BLN10 + $list_bgt->MON_BLN11 + $list_bgt->MON_BLN12 +
                                                $list_bgt->MON_BLN13 + $list_bgt->MON_BLN14 + $list_bgt->MON_BLN15;
                                        echo "<td align='right' style='font-weight:bold;'>Rp " . number_format($total_plan, '2', ',', '.') . "</td>";
                                        ...
                                        $prop_data = $bgt_aii->query("SELECT TOP 1 * FROM BDGT_TT_MASTER_CHANGELOG WHERE CHR_NO_BUDGET = '$list_bgt->CHR_NO_BUDGET' AND CHR_FLG_SWITCH = '0' ORDER BY INT_NO_REVISI DESC");
                                        $total_propose = 0;
                                        if($prop_data->num_rows() > 0){
                                            $data_prop = $prop_data->row();
                                            $total_propose = $data_prop->MON_LIMBLN01 + $data_prop->MON_LIMBLN02 + $data_prop->MON_LIMBLN03 + $data_prop->MON_LIMBLN04 +
                                                        $data_prop->MON_LIMBLN05 + $data_prop->MON_LIMBLN06 + $data_prop->MON_LIMBLN07 + $data_prop->MON_LIMBLN08 +
                                                        $data_prop->MON_LIMBLN09 + $data_prop->MON_LIMBLN10 + $data_prop->MON_LIMBLN11 + $data_prop->MON_LIMBLN12;
                                        }
                                        echo "<td align='right' style='font-weight:bold;'>Rp " . number_format($total_propose, '2', ',', '.') . "</td>";
                                        ...
                                        if($total_propose == '0'){
                                            if($total_amount > $total_plan){
                                                echo "<td align='center' style='color:red;'>OVER BUDGET</i></td>";
                                            } else if($total_amount == $total_plan) {
                                                echo "<td align='center'>ON BUDGET</i></td>";
                                            } else {
                                                echo "<td align='center'>UNDER BUDGET</i></td>";
                                            }                                            
                                        } else {
                                            if($total_propose > $total_plan){
                                                echo "<td align='center' style='color:red;'>OVER BUDGET</i></td>";
                                            } else if($total_propose == $total_plan) {
                                                echo "<td align='center'>ON BUDGET</i></td>";
                                            } else {
                                                echo "<td align='center'>UNDER BUDGET</i></td>";
                                            }  
                                        ...
                                        if ($list_bgt->CHR_KODE_TYPE_BUDGET == 'CAPEX'){
                                            if ($list_bgt->CHR_FLG_USED == '1'){
                                                ...
                                                echo "<td align='center'><a href='" . base_url('index.php/budget/propose_budget_c/edit_budget_revision') . "/" . $fiscal_start . "/" . $budget_type . "/" . str_replace('/', '<', trim($list_bgt->CHR_NO_BUDGET)) . "' class='label label-primary' data-placement='left' title='Edit'><span class='fa fa-pencil' ></span></a></td>";
                                            }
                                        } else {
                                            echo "<td align='center'><a href='" . base_url('index.php/budget/propose_budget_c/edit_budget_revision') . "/" . $fiscal_start . "/" . $budget_type . "/" . str_replace('/', '<', trim($list_bgt->CHR_NO_BUDGET)) . "' class='label label-primary' data-placement='left' title='Edit'><span class='fa fa-pencil' ></span></a></td>";
                                        }                                        
