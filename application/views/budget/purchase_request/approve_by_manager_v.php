    ...
    <section class="content-header">
        <ol class="breadcrumb">
            ...
            <?php if ($session['ROLE'] == 4) { ?>
                <li><a href ="<?php echo base_url('index.php/budget/purchase_request_c/prepare_approve_purchase_request') ?>">Department List</a></li>
            <?php } if ($session['ROLE'] == 3) { ?>
                <li><a href ="<?php echo base_url('index.php/budget/purchase_request_c/prepare_approve_purchase_request') ?>">GM List</a></li>
                <li><a href ="<?php echo base_url('index.php/budget/purchase_request_c/select_by_gm/' . $id_dept . '/' . $id_fiscal) ?>">Department List</a></li>
            <?php } ?>
            ...
            <div class="col-md-6">
                <div class="grid">
                    <div class="grid-header">
                        ...
                        <span class="grid-title"><strong>REALIZATION</strong> <?php echo $group . ' [' . $fiscal . ']' ?> </span>
                        ...
                        <table id="dataTables1" class="table table-condensed table-hover display" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                ...
                                foreach ($data as $isi) {
                                    ...
                                    $total_plan = $this->capex_plan_m->get_total_capex_plan_by_supervisor($isi->INT_ID_SECTION, $isi->INT_ID_FISCAL_YEAR);
                                    echo "<td>" . 'Rp ' . number_format($total_plan, 0, '', '.') . "</td>";
                                    echo "<td>" . 'Rp ' . number_format($isi->total, 0, '', '.') . "</td>";
                                    ?>
    <!--                                <td>
                                        <a href="<?php echo base_url('index.php/budget/  purchase_request_c/view_detail_purchase_request') . "/" . $isi->INT_ID_SECTION . "/" . $isi->INT_ID_FISCAL_YEAR; ?>" class="label label-success" data-toggle="tooltip" data-placement="right" title="View"><span class="fa fa-check"></span></a>
                                    </td>-->
                                    </tr>
                                    <?php
                                    $i++;
                        ...
                        <strong>Total budget Actual <?php echo $group; ?> Dept:</strong> <span style="background-color: yellow;"> Rp <?php echo number_format($total_real, 0, '', '.'); ?></span><br>
...
<?php
foreach ($data as $row) {
    $total_plan = $this->capex_plan_m->get_total_capex_plan_by_supervisor($row->INT_ID_SECTION, $row->INT_ID_FISCAL_YEAR);
    ?>
        {
        name: '<?php echo $row->CHR_SECTION; ?>',
                data: [<?php echo $total_plan ?>,<?php echo $row->total ?> ]
        },
<?php } ?>
    ]
    });
            ...
            <div class="col-md-6">
                <div class="grid">
                    <div class="grid-header">
                        ...
                        <span class="grid-title"><strong>REALIZATION</strong> <?php echo $group ?>  CHART</span>
                        <div class="pull-right grid-tools">
                            <a data-widget="collapse" title="Collapse" data-toggle="tooltip" data-placement="left" ><i class="fa fa-chevron-up"></i></a>
                        </div>
                    </div>
...
foreach ($data_pureq_detail as $row) {
    ?>
                    {
                    name: '<?php echo $row->INT_NO_PUREQ . '[' . $row->CHR_SECTION . ']'; ?>',
