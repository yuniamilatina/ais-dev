...
<aside class="right-side">
    <section class="content-header">
        <ol class="breadcrumb">
            ...
            <li><a href=""><strong>REPORT OVERTIME</strong></a></li>
        </ol>
    </section>
...
foreach ($overtime_summary as $plan) {
    if ($row_plan == $count_over_plan) {
        if($plan->OBG <= 0){
            echo '{label: "' . $plan->KD_SUB_SECTION . '", y:' . 0 . '}';
        }else{
            echo '{label: "' . $plan->KD_SUB_SECTION . '", y:' . $plan->OBG . ', indexLabel:"' . number_format($plan->OBG, 0, ",", ".") . '"}';
        }
    } else {
        if($plan->OBG <= 0){
            echo '{label: "' . $plan->KD_SUB_SECTION . '", y:' . 0 . '},';
...
foreach ($overtime_summary as $quota) {
    if ($row_quo == $count_over_quota) {
        echo '{label: "' . $quota->KD_SUB_SECTION . '", y:' . $quota->QUOTA_STD . ', indexLabel:"' . number_format($quota->QUOTA_STD, 0, ",", ".") . '"}';
    } else {
        echo '{label: "' . $quota->KD_SUB_SECTION . '", y:' . $quota->QUOTA_STD . ', indexLabel:"' . number_format($quota->QUOTA_STD, 0, ",", ".") . '"},';
        $row_quo++;
    }
...
foreach ($overtime_summary as $quota) {
    if ($row_quo == $count_over_quota) {
        echo '{label: "' . $quota->KD_SUB_SECTION . '", y:' . ($quota->QUOTA_PLAN + $quota->QUOTA_PLAN1) . ', indexLabel:"' . number_format(($quota->QUOTA_PLAN + $quota->QUOTA_PLAN1), 0, ",", ".") . '"}';
    } else {
        echo '{label: "' . $quota->KD_SUB_SECTION . '", y:' . ($quota->QUOTA_PLAN + $quota->QUOTA_PLAN1) . ', indexLabel:"' . number_format(($quota->QUOTA_PLAN + $quota->QUOTA_PLAN1), 0, ",", ".") . '"},';
        $row_quo++;
    }
...
foreach ($overtime_summary as $actual) {
    if ($row_act == $count_over_actual) {
        echo '{label: "' . $actual->KD_SUB_SECTION . '", y:' . $actual->TOT_DURASI_OVERTIME . ', indexLabel:"' . number_format($actual->TOT_DURASI_OVERTIME, 0, ",", ".") . '"}';
    } else {
        echo '{label: "' . $actual->KD_SUB_SECTION . '", y:' . $actual->TOT_DURASI_OVERTIME . ', indexLabel:"' . number_format($actual->TOT_DURASI_OVERTIME, 0, ",", ".") . '"},';
        $row_act++;
    }
...
<!--                             <table width="100%" id='filter'>
                                <tr>
                                    ...
                                    <td width="60%">
                                        <select class="ddl" id="chart_type" name="chart_type" onChange="document.location.href = this.options[this.selectedIndex].value;">
                                            <option value="1"> Amount </option>
                                            <option valsue="2"> Hour </option>
                                        </select>
                                    </td>
            ...
            <div class="col-md-12">
                <div class="grid">
                    <div class="grid-header">
                        ...
                        <span class="grid-title" style=" font-weight: bold">REPORT OVERTIME BY LINE</span>
                        <div class="pull-right grid-tools">
                            <a data-widget="collapse" title="Collapse"><i class="fa fa-chevron-up"></i></a>
                        </div>
                        <div class="pull-right grid-tools">
                            <?php echo form_open('aorta/report_overtime_c/print_report_overtime', 'class="form-horizontal"'); ?>
                    ...
                    <div class="grid-body">
                            ...
                            <table width="100%" id='filter'>
                                <tr>
                                    ...
                                    <td width="40%">
                                        <select class="ddl" id="tanggal" onChange="document.location.href = this.options[this.selectedIndex].value;">
                                            <?php for ($x = -10; $x <= 0; $x++) {  $y = $x * 28 ?>
                                                <option value="<?PHP echo site_url('aorta/report_overtime_c/overtime/' . date("Ym", strtotime("+$y day")) . "/$id_prod/$id_section/$id_subsect"); ?>" <?php
                                    ...
                                    <td width="40%">
                                        <select onChange="document.location.href = this.options[this.selectedIndex].value;" class="ddl2">
                                            <option value="<?php echo site_url('aorta/report_overtime_c/overtime/' . $selected_date . "/$id_prod/ALL/ALL") ?>">ALL</option>
                                            <?php foreach ($all_work_centers as $row) { ?>
                                                <option value="<?php echo site_url('aorta/report_overtime_c/overtime/' . $selected_date . "/$id_prod/".trim($row->KODE)."/ALL"); ?>" 
                                                <?php
                                ...
                                <tr>
                                    ...
                                    <td width="40%">
                                            ...
                                            <select disabled style="cursor:not-allowed;background-color: #F3F4F5;" onChange="document.location.href = this.options[this.selectedIndex].value;" class="ddl2">
                                                <?php } else { ?>
                                                <select onChange="document.location.href = this.options[this.selectedIndex].value;" class="ddl2">
                                                <?php } ?>
                                                    <option value="">-- Select Dept --</option>
                                                <?php foreach ($all_dept_prod as $row) { ?>
                                                    <option value="<?php echo site_url("aorta/report_overtime_c/overtime/" . $selected_date . '/' . $row->INT_ID_DEPT . "/ALL/ALL"); ?>" <?php
                                    ...
                                    <td width="10%">Sub Section</td>
                                    <td  width="40%">
                                        <select onChange="document.location.href = this.options[this.selectedIndex].value;" class="ddl2">
                                            <option value="<?php echo site_url('aorta/report_overtime_c/overtime/' . $selected_date . "/$id_prod/$id_section/ALL") ?>">ALL</option>
                                            <?php foreach ($all_lines as $row) { ?>
                                                <option value="<?php echo site_url('aorta/report_overtime_c/overtime/' . $selected_date . "/$id_prod/$id_section/$row->KODE"); ?>" 
                                                <?php
                        ...
                        <div id="table-luar">
                            <table id="example" class="table table-condensed table-bordered table-striped table-hover display" cellspacing="0" width="100%" >
                                <thead>
                                    <tr>
                                        <th rowspan="2" style="vertical-align: middle;">Sub Section (Line)</th>
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($choose_line as $value) { ?>
                                        ...
                                        <tr>
                                            ...
                                            <?php
                                            $actual_hour = $aortadb->query("EXEC zsp_get_actual_overtime_hour '$selected_date', '" . $value->KD_DEPT . "' , '" . $value->KD_SECTION . "','" . $value->KD_SUB_SECTION . "'")->row();
                                        ...
                                        <tr>
                                            ...
                                            <?php
                                            $actual_amount = $aortadb->query("EXEC zsp_get_actual_overtime_amount '$selected_date', '" . $value->KD_DEPT . "' , '" . $value->KD_SECTION . "', '" . $value->KD_SUB_SECTION . "'")->row();
                                        ...
                                        <tr>
                                            <td rowspan="" style="text-align:center; background-color: #F3F4F5; display: none;">
                                                <?php echo $value->KD_SUB_SECTION; //$value->NAMA_SUBSECT;    ?>
                                            </td>
                                            <td rowspan="" style="text-align:center; background-color: #F3F4F5; display: none;">
                                                Accumulative Plan
                                            </td>
