            ...
            <div class="col-md-12">
                <div class="grid">
                    ...
                    <div class="grid-body">
                                    ...
                                    <td width="10%">
                                        <select class="ddl" id="tanggal" onChange="document.location.href = this.options[this.selectedIndex].value;">
                                            <?php for ($x = -24; $x <= 1; $x++) { $y = $x * 28 ?>
                                                <option value="<? echo site_url('aorta/quota_employee_c/prepare_approve_quota_by_gm/' . date("Ym", strtotime("+$y day")) . '/' . $dept); ?>" <?php
                                    ...
                                    <td width="10%" style='text-align:left;' colspan="4">
                                        <select id="opt_wcenter" onChange="document.location.href = this.options[this.selectedIndex].value;" class="ddl">
                                            ...
                                            <?php foreach ($all_dept as $row) { ?>
                                                <option value="<? echo site_url('aorta/quota_employee_c/prepare_approve_quota_by_gm/' . $period . '/' . $row->KODE); ?>" <?php
                                                if ($dept == $row->KODE) {
                                                    echo 'SELECTED';
                                                }
                        ...
                        <?php echo form_open('aorta/quota_employee_c/approve_all_quota_employee_by_gm', 'class="form-horizontal"'); ?>
                        ...
                        <div id="table-luar">
                            <table id="dataTables2" class="table table-condensed table-striped table-hover display" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    ...
                                    $i = 1;
                                    foreach ($data as $isi) {
                                        if ($isi->KADEP_APPROVE == 1 && $isi->GM_APPROVE == 0) {
                                            $color = '#FE2D45';
                                            $display = 'enabled';
                                        }
                                        else if ($isi->KADEP_APPROVE == 1 && $isi->GM_APPROVE == 1) {
                                            $color = '#55D785';
                                            $display = 'disabled';
                                        }
                        ...
                        <span style="font-size:9pt"><!--* only exceeds the standard quota --></span> <!--(40 hours) -->
                    </div>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header  bg-primary">
                                            <?php echo form_open('aorta/quota_employee_c/approve_form_quota_employee_by_gm', 'class="form-horizontal"'); ?>

                                            <input name="ID_DOC" class="form-control" required type="hidden" style="width: 300px;" value="<?php echo $isi->ID_DOC ?>">
                                            <input name="CHR_DEPT" class="form-control" required type="hidden" style="width: 300px;" value="<?php echo trim($dept) ?>">
        ...
        <div class="row">
            <div class="col-md-8">
                <div class="grid">
                    ...
                    <div class="grid-body">
                        <iframe frameBorder="0" width='100%' height='300px' src="<?php echo site_url("aorta/overtime_c/view_detail_ot_section_by_gm/" . $period . "/" . $dept); ?>"></iframe>
                    </div>
                </div>
            </div>
                            ...
                            <table class="table table-condensed table-hover display" cellspacing="0" width="50%" style="font-size: 11px;">
                                ...
                                <tbody>
                                    <tr>
                                        <td>Total MP</td>
                                        <td>:</td>
                                        <td><strong> <?php if($detail_quota_group){ echo $detail_quota_group->KRY; }else{ echo 0; }?>  MP</strong></td>
                                    </tr>
                                    <tr>
                                        <td>Quota STD</td>
                                        <td>:</td>
                                        <td><strong><?php if($detail_quota_group){ echo number_format($detail_quota_group->QUOTA_STD, 2, ',', '.'); }else{ echo 0; }?> H</strong></td>
                                    </tr>
                                    <tr>
                                        <td>Quota Plan</td>
                                        <td>:</td>
                                        <td><strong><?php if($detail_quota_group){ echo number_format(($detail_quota_group->QUOTAPLAN + $detail_quota_group->QUOTAPLAN1), 2, ',', '.'); }else{ echo 0; }?> H</strong></td>
                                    </tr>
                                    <tr style="background-color: whitesmoke;">
                                        <td>Actual OT</td>
                                        <td>:</td>
                                        <td><strong><?php if($detail_quota_group){ echo number_format(($detail_quota_group->TERPAKAIPLAN + $detail_quota_group->TERPAKAIPLAN1), 2, ',', '.'); }else{ echo 0; }?> H</strong></td>
                                    </tr>
                                    <tr>
                                        <td>Saldo Quota</td>
                                        <td>:</td>
                                        <td><strong><?php if($detail_quota_group){ echo number_format(($detail_quota_group->SISAPLAN + $detail_quota_group->SISAPLAN1), 2, ',', '.'); }else{ echo 0; }?> H</strong></td>
                                    </tr>
                                    <tr>
                                        <td>Avg OT/MP</td>
                                        <td>:</td>
                                        <td><strong><?php if($detail_quota_group){ echo number_format((($detail_quota_group->TERPAKAIPLAN + $detail_quota_group->TERPAKAIPLAN1)/$detail_quota_group->KRY), 2, ',', '.'); }else{ echo 0; }?> H</strong></td>
                                    </tr>
                                    <tr>
                                        <td>Avg Quota (Est 22 WD)</td>
                                        <td>:</td>
                                        <td><strong><?php if($detail_quota_group){ echo number_format(($detail_quota_group->AVG_QUOTA + $detail_quota_group->AVG_QUOTA1), 2, ',', '.'); }else{ echo 0; }?> H/Day</strong></td>
                                    </tr>
                                    <tr>
                                        <td>Avg OT (Est 22 WD)</td>
                                        <td>:</td>
                                        <td><strong><?php if($detail_quota_group){ echo number_format(($detail_quota_group->AVG_OT + $detail_quota_group->AVG_OT1), 2, ',', '.'); }else{ echo 0; }?> H/Day</strong></td>
                                    </tr>
                                    <tr>
                                        <td>Avg Saldo (Est 22 WD)</td>
                                        <td>:</td>
                                        <td><strong><?php if($detail_quota_group){ echo number_format(($detail_quota_group->AVG_SISA + $detail_quota_group->AVG_SISA1), 2, ',', '.'); }else{ echo 0; }?> H/Day</strong></td>
                                    </tr>   
                                    <tr style="background-color: whitesmoke;">
                                        <td>Budget Quota</td>
                                        <td>:</td>
                                    ...
                                    <?php 
                                    $act_vs_budget = (($detail_quota_group->TERPAKAIPLAN + $detail_quota_group->TERPAKAIPLAN1)/$detail_quota_group->INT_BUDGET_QUOTA)*100;
                                    $style_color = '';
                                    if($act_vs_budget <= 90){
                                    ...
                                    <tr style="<?php echo $style_color; ?>">
                                        ...
                                        <td>
                                            ...
                                            <a  data-toggle="modal" data-target="#modalQuota" data-placement="left" data-toggle="tooltip" title="Quota Usage per Dept" class="label label-info"><span class="fa fa-search"></span></a>
                                                ...
                                                <!--<iframe frameBorder="0" width='100%' height='250px' src="<?php //echo site_url("aorta/overtime_c/view_detail_ot_group/" . $period . "/" . $group);  ?>"></iframe>-->
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="grid">
                    ...
                    <div class="grid-body">
                        <iframe frameBorder="0" width='100%' height='220px' src="<?php echo site_url("aorta/overtime_c/view_detail_ot_per_month_gm/" . $period . "/" . $group); ?>"></iframe>
                    </div>
                </div>
            </div>
        </div>
...
<script>
      ...
      function get_data_detail(qrno) {
          ...
          $.ajax({
              async: false,
              type: "POST",
              url: "<?php echo site_url('aorta/quota_employee_c/view_detail_quota_employee_by_gm'); ?>",
              data: "qrno=" + qrno,
