...
<aside class="right-side">
    <section class="content-header">
        <ol class="breadcrumb">
            ...
            <li><a href=""><strong>REPORT OVERTIME BY DEPARTMENT</strong></a></li>
        </ol>
    </section>
...
foreach ($overtime_summary as $plan) {
    if ($row_plan == $count_over_plan) {
        if ($plan->OBG <= 0) {
            echo '{label: "' . $plan->KD_DEPT . '", y:' . 0 . '}';
        } else {
            echo '{label: "' . $plan->KD_DEPT . '", y:' . $plan->OBG . ', indexLabel:"' . number_format($plan->OBG, 0, ",", ".") . '"}';
        }
    } else {
        if ($plan->OBG <= 0) {
            echo '{label: "' . $plan->KD_DEPT . '", y:' . 0 . '},';
...
foreach ($overtime_summary as $quota) {
    if ($row_quo == $count_over_quota) {
        echo '{label: "' . $quota->KD_DEPT . '", y:' . $quota->QUOTA_STD . ', indexLabel:"' . number_format($quota->QUOTA_STD, 0, ",", ".") . '"}';
    } else {
        echo '{label: "' . $quota->KD_DEPT . '", y:' . $quota->QUOTA_STD . ', indexLabel:"' . number_format($quota->QUOTA_STD, 0, ",", ".") . '"},';
        $row_quo++;
    }
...
foreach ($overtime_summary as $quota) {
    if ($row_quo == $count_over_quota) {
        echo '{label: "' . $quota->KD_DEPT . '", y:' . ($quota->QUOTA_PLAN + $quota->QUOTA_PLAN1) . ', indexLabel:"' . number_format(($quota->QUOTA_PLAN + $quota->QUOTA_PLAN1), 0, ",", ".") . '"}';
    } else {
        echo '{label: "' . $quota->KD_DEPT . '", y:' . ($quota->QUOTA_PLAN + $quota->QUOTA_PLAN1) . ', indexLabel:"' . number_format(($quota->QUOTA_PLAN + $quota->QUOTA_PLAN1), 0, ",", ".") . '"},';
        $row_quo++;
    }
...
foreach ($overtime_summary as $actual) {
    if ($row_act == $count_over_actual) {
        echo '{label: "' . $actual->KD_DEPT . '", y:' . $actual->TOT_DURASI_OVERTIME . ', indexLabel:"' . number_format($actual->TOT_DURASI_OVERTIME, 0, ",", ".") . '"}';
    } else {
        echo '{label: "' . $actual->KD_DEPT . '", y:' . $actual->TOT_DURASI_OVERTIME . ', indexLabel:"' . number_format($actual->TOT_DURASI_OVERTIME, 0, ",", ".") . '"},';
        $row_act++;
    }
            ...
            <div class="col-md-12">
                <div class="grid">
                    <div class="grid-header">
                        ...
                        <span class="grid-title"><strong>REPORT OVERTIME BY DEPARTMENT</strong></span>
                        <div class="pull-right grid-tools">
                            <a data-widget="collapse" title="Collapse"><i class="fa fa-chevron-up"></i></a>
                        </div>
                        <div class="pull-right grid-tools">
                        ...
                        <div class="pull">
                            ...
                            <table width="100%" id='filter'>
                                <tr>
                                    <td width="10%">
                                        <select class="ddl" id="tanggal" onChange="document.location.href = this.options[this.selectedIndex].value;">
                                            <?php for ($x = -5; $x <= 0; $x++) {  $y = $x * 28 ?>
                                                <option value="<?PHP echo site_url('aorta/report_overtime_perdept_c/index/' . date("Ym", strtotime("+$y day")) . "/$id_dept"); ?>" <?php
                                    ...
                                    <td width="10%">
                                        <?php //if ($role == 5 || $role == 6 || $role == 39 || $role == 45) { ?>
                                            ...
                                            <?php //} else { ?>
                                                ...
                                                <?php //} ?>
                                                <option value="<?php echo site_url('aorta/report_overtime_perdept_c/index/' . $selected_date . "/ALL") ?>">ALL</option>
                                                <?php foreach ($all_dept_prod as $row) { ?>
                                                    <option value="<?php echo site_url("aorta/report_overtime_perdept_c/index/" . $selected_date . '/' . $row->INT_ID_DEPT); ?>" <?php
                                    ...
                                    <?php echo form_open('aorta/report_overtime_perdept_c/print_report_overtime', 'class="form-horizontal"'); ?>
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($choose_dept as $value) { ?>
                                        ...
                                        <tr>
                                            ...
                                            <?php
                                            $actual_hour = $aortadb->query("EXEC zsp_get_actual_overtime_hour_dept '$selected_date', '" . $value->KD_DEPT . "'")->row();
                                        ...
                                        <tr>
                                            ...
                                            <?php
                                            $actual_amount = $aortadb->query("EXEC zsp_get_actual_overtime_amount_dept '$selected_date', '" . $value->KD_DEPT . "'")->row();
                                        ...
                                        <tr>
                                            <td rowspan="" style="text-align:center; background-color: #F3F4F5; display: none;">
    <?php echo $value->KD_DEPT; //$value->NAMA_SUBSECT;       ?>
                                            </td>
                                            <td rowspan="" style="text-align:center; background-color: #F3F4F5; display: none;">
                                                Accumulative Plan
                                            </td>
